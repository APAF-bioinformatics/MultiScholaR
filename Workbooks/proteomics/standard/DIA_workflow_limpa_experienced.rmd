...
# Note: For data searched with match-between-run (MBR), use q.columns = c("Q.Value","Lib.Q.Value","Lib.PG.Q.Value")
# For data searched without MBR, use q.columns = c("Q.Value","Global.Q.Value","Global.PG.Q.Value")
...
## Average Technical Replicates for Differential Expression Analysis
...
4. **Prepares for DE Analysis**: Creates a cleaner dataset where each column represents an independent biological sample, meeting the assumptions of differential expression statistical models

**Important Note**: This averaging is appropriate for technical replicates only. Biological replicates should never be averaged before differential expression analysis, as biological variation is the signal of interest.
...
- [Limma for Proteomics](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4402510/) - Kammers et al. (2015) comparison of methods for differential expression analysis
...
1. **Iterates Through Contrasts** (`imap_dfr`): Processes each differential expression comparison separately while preserving comparison names
