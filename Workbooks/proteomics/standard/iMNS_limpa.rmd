...
source(file.path( here::here(), "scripts", "proteomics",  "plotPca.R") ) 
p_load(conflicted)
p_load(tidytext)
...
QC_composite_figure@limpa_plots$limpa_qc_plots <- limpa_qc_plots
...
## Average biological replicates for DE analysis
```{r}

ruv_normalised_filtered_results_obj@technical_replicate_id <- "factor2"  # Grouping by Sample ID minus the rep suffix

ruv_normalised_for_de_analysis_obj <- averageTechReps(
    ruv_normalised_filtered_results_obj,
    design_matrix_columns = c("group", "factor2")  # Keep relevant metadata columns
)
...
source( file.path(here::here(), "scripts", "proteomics", "extra_scripts.R") )
pca_ruv_rle_correlation_merged <- createGridQC(
  ...
  save_path = file.path(project_dirs[[paste0(omic_type, "_", experiment_label)]]$protein_qc_dir),
    pca_titles = c("a)", "b)", "c)", "d)", "e)"),
  density_titles = c("a)", "b)", "c)", "d)", "e)"),
  rle_titles = c("a)", "b)", "c)", "d)", "e)"),
  pearson_titles = c("a)", "b)", "c)", "d)", "e)"),
...
# swap out factor1 ~~ group to avoid unnecessary refactoring (revisit this later)
...
## StringDB enrichment analysis can be performed separately using the stringDB_enrichment_analysis.rmd script
```{r}
# names( de_analysis_results_list )
# 
# names( de_analysis_results_list[[1]]$de_proteins_long )
# 


source(file.path( here::here(), "scripts/proteomics/rank_enrichment_stringdb.R") )

enrichment_cache_file <- file.path( results_dir, "functional_enrichment_string_db", "all_enrichment_results.rds" )
...
results_dir <- project_dirs[[paste0(omic_type, "_", experiment_label)]]$results_dir
output_group_tables_list <- purrr::map ( list_of_contrasts, \(contrast_name) { print(contrast_name) 
  
  input_table <- de_analysis_results_list[[ contrast_name ]]$de_proteins_long
  result_label <- str_split_i(contrast_name, "=", 1)
  
   runOneStringDbRankEnrichment( input_table , result_label, api_key="bjcR4Px5rByQ"  ) 
  })


names(output_group_tables_list) <- purrr::map_chr( list_of_contrasts
                , \(x) {
                  paste( str_split(x, "_")[[1]][1], "(M-C)")
                })

output_group_main_table <- bind_rows( output_group_tables_list
           , .id = "comparison") 


saveRDS(output_group_main_table, file = enrichment_cache_file)
vroom::vroom_write(output_group_main_table, file.path(results_dir, "functional_enrichment_string_db", "all_enrichment_results.tsv"))
}
...
# 5. Plot results
# This part now checks if the RDS exists implicitly by checking the loaded data frame.

# Define the path to the cached enrichment results
results_dir <- project_dirs[[paste0(omic_type, "_", experiment_label)]]$results_dir
enrichment_cache_file <- file.path( results_dir, "functional_enrichment_string_db", "all_enrichment_results.rds" )
enrichment_dir <-  file.path( results_dir, "functional_enrichment_string_db")
# Check if the cache file exists and load it
if (file.exists(enrichment_cache_file)) {
  all_enrichment_results <- readRDS(enrichment_cache_file)
  
  cat("Loaded", nrow(all_enrichment_results), "rows from cached enrichment results.\n")

  # The rest of the processing and plotting code from the original chunk
  sorted_enrichment_results <- all_enrichment_results |> 
    group_by( comparison, category) |> 
    arrange( comparison, category, desc(enrichmentScore), falseDiscoveryRate) |> 
    mutate( rank = row_number( )) |> 
    ungroup() 
    
  included_functional_category_and_term <- sorted_enrichment_results |> 
    dplyr::filter(rank<=5) |> 
    distinct( category, termID)
    
  filtered_enrichment_results <- sorted_enrichment_results |> 
      inner_join( included_functional_category_and_term
      , by = join_by(category, termID)) |>
      arrange(  comparison, category, desc(enrichmentScore), falseDiscoveryRate) 

  vroom::vroom_write(filtered_enrichment_results, file.path(enrichment_dir, "filtered_enrichment_results.tsv"))


  # The ordering is now handled by the plotting function `printStringDbFunctionalEnrichmentBarGraph`

  # Define category groups for splitting the plot
  # Using str_detect for GO terms to be more robust
  ...
  results_fig1 <- filtered_enrichment_results |> 
    filter(str_detect(category, fig1_pattern))

  results_fig2 <- filtered_enrichment_results |> 
    filter(category %in% fig2_categories)

  results_fig3 <- filtered_enrichment_results |> 
    filter(category %in% fig3_categories)

  results_fig4 <- filtered_enrichment_results |> 
    filter(!str_detect(category, fig1_pattern) & !category %in% fig2_categories  & !category %in% fig3_categories)



  # Helper function to generate, save, and print each plot
  plot_and_save <- function(data, filename_suffix, word_limit = 7, width = 16, height = 12) {
    if (nrow(data) > 0) {
      cat(paste("\n--- Generating Enrichment Plot for", filename_suffix, "---\n"))

      enrichment_plot <- printStringDbFunctionalEnrichmentBarGraph(data, word_limit = word_limit)

      # Save plot
      output_filename <- file.path(enrichment_dir, paste0("enrichment_summary_", filename_suffix))
      cat(paste("Saving plot to:", paste0(output_filename, ".png"), "\n"))

      purrr::walk( c( ".png", ".pdf"), \(x) { 
      ggsave(
        plot = enrichment_plot,
        filename = paste0(output_filename, x),
        width = width,
        height = height,
        dpi = 300
      ) })
      ...
      cat(paste("\nNo enrichment results to plot for", filename_suffix, "\n"))
    }
  }

  # Generate the three plots
  plot_and_save(results_fig1, "go_terms")
  plot_and_save(results_fig2, "pathways")
  plot_and_save(results_fig3, "protein_domains")
  plot_and_save(results_fig4, "others")

} else {
  cat("Enrichment results file not found. Please run the 'stringdb-enrichment' chunk first.\n")
}
