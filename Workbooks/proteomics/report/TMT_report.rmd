...
ExtractStudyParametersTMT <- function(study_params) {
  ...
  all_params <- list(
    ...
    # TMT Protein-level filtering parameters
    groupwise_percentage_cutoff = as.numeric(s4_prot_filt$groupwise_percentage_cutoff %||% 60),
    max_groups_percentage_cutoff = as.numeric(s4_prot_filt$max_groups_percentage_cutoff %||% 60),
    proteins_intensity_cutoff_percentile = as.numeric(s4_prot_filt$proteins_intensity_cutoff_percentile %||% 1),
    ...
    # FASTA Processing
    fasta_format = fasta_info$"Format",
    fasta_num_sequences = fasta_info$"Sequences",
    fasta_has_protein_evidence = fasta_info$"Protein Evidence Available",
    fasta_has_gene_names = fasta_info$"Gene Names Available",
    fasta_has_isoform_info = fasta_info$"Isoform Information Available",
    
    # Accession Cleanup
    accession_cleanup_applied = accession_cleanup$"Cleanup Applied",
    accession_aggregation_method = accession_cleanup$"Aggregation Method",
    accession_delimiter = accession_cleanup$"Delimiter Used",
    accession_proteins_before = accession_cleanup$"Proteins Before Cleanup",
    accession_proteins_after = accession_cleanup$"Proteins After Cleanup",
    accession_had_full_metadata = accession_cleanup$"Full UniProt Metadata",
...
FormatMethodDetailsTMT <- function(numbers) {
  # Use TMT-specific parameters
  groupwise_cutoff <- numbers$groupwise_percentage_cutoff %||% 60
  max_groups_cutoff <- numbers$max_groups_percentage_cutoff %||% 60
  intensity_cutoff_pct <- numbers$proteins_intensity_cutoff_percentile %||% 1
...
{if (!is.null(numbers$fasta_format) && !is.na(numbers$fasta_format)) {
  if (numbers$fasta_format == "standard_uniprot") {
    ...
    glue::glue("Protein identifications were validated against a {numbers$fasta_format} FASTA database containing {numbers$fasta_num_sequences} sequences. Note that comprehensive UniProt metadata (protein evidence levels, reviewed status) was not available for this reference database, and accession selection was based primarily on sequence length and alphabetical ordering.")
  }
} else {
  "Protein identifications were validated against the provided FASTA reference database to ensure accurate annotation."
}}

{if (!is.null(numbers$accession_cleanup_applied) && isTRUE(as.logical(numbers$accession_cleanup_applied))) {
  metadata_basis <- if (!is.null(numbers$fasta_has_protein_evidence) && numbers$fasta_has_protein_evidence == "TRUE") {
    "protein evidence levels, sequence length, and database annotation quality"
  } else {
    "sequence length and alphabetical ordering"
  }
  glue::glue("Protein groups containing multiple accessions (separated by '{numbers$accession_delimiter}' delimiters) were resolved by selecting the best representative protein based on {metadata_basis}. Quantitative values for proteins within the same group were aggregated using the '{numbers$accession_aggregation_method}' method, reducing {numbers$accession_proteins_before} protein groups to {numbers$accession_proteins_after} unique protein identifications.")
} else {
...
}} Duplicate protein entries were removed to prevent redundancy in quantification.
...
testing. A protein was excluded if it had {groupwise_cutoff}% or more missing 
values within individual experimental groups, or if it had {max_groups_cutoff}% 
or more missing values across the maximum number of groups. Additionally, 
proteins with intensity values below the {intensity_cutoff_pct}st percentile 
were excluded from further analysis as likely noise or background signal.
...
# UNCHANGED: Results function - focuses on correlation data (same for TMT and DIA)
