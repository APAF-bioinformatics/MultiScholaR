...
# UPDATED to include more dynamic sample information
...
# UPDATED to use newly extracted, detailed parameters
FormatMethodDetails <- function(numbers) {
  # Use the safe default operator for all parameters
  ...
  # New RUV parameters from "Automatic RUV Optimization Results"
  num_neg_ctrl_val <- numbers$num_neg_ctrl %||% "an automatically determined number of"
  best_k_val <- numbers$best_k %||% "an automatically determined number of"
  best_pct_val <- numbers$best_percentage %||% "an automatically determined percentage of"
...
# UPDATED to use UI parameters for a more accurate description
FormatDEDetails <- function(numbers) {
  # Get parameters from UI if available, otherwise from S4 args, with defaults
  treat_lfc_cutoff_val <- numbers$ui_de_treat_lfc_cutoff %||% numbers$s4_de_treat_lfc_cutoff %||% 0
  de_q_val_thresh_val <- numbers$ui_de_q_value_threshold %||% numbers$s4_de_q_val_thresh %||% 0.05
  treat_enabled_val <- numbers$ui_de_treat_enabled %||% (treat_lfc_cutoff_val > 0)
  ...
  treat_desc <- if (isTRUE(treat_enabled_val) && treat_lfc_cutoff_val > 0) {
    glue::glue("The TREAT method was used to test against a log-fold-change threshold of {treat_lfc_cutoff_val}.")
  } else {
...
with q-values less than {de_q_val_thresh_val}.

Volcano plots of differentially expressed proteins across all groups are shown 
in Figure 2, with a threshold of q-value < {de_q_val_thresh_val} and no log fold-change 
...
# UPDATED to use extracted organism info and UI parameters
FormatGOEnrichment <- function(numbers) {
  # Use new safe extraction, providing defaults
  organism_name_val <- numbers$organism_name %||% "the species under study"
  taxon_id_val <- numbers$taxon_id %||% NA
  
  # Get UI params for enrichment
...
# --- REWRITTEN ---
# This entire chunk has been replaced with a robust parser and updated extraction logic.

# Helper function for safely getting nested list values
safe_get <- function(l, path, default = NULL) {
  # path is a character vector of keys
  val <- purrr::reduce(path, function(acc, key) {
    if (is.list(acc) && key %in% names(acc)) {
      acc[[key]]
    } else {
      NULL # Return NULL if any key is not found
    }
  }, .init = l)
...
# New robust parameter extraction function
ExtractStudyParameters <- function(study_params) {
  
  # --- S4 Workflow Parameters ---
  s4_params <- safe_get(study_params, c("Workflow Parameters", "Parameters from Final S4 Object"), default = list())
  ...
  s4_qval_filt <- s4_params$srlQvalueProteotypicPeptideClean %||% list()
  s4_sample_filt <- s4_params$filterMinNumPeptidesPerSample %||% list()
  
  # --- UI Parameters ---
  ui_params <- safe_get(study_params, c("Workflow Parameters", "User Interface Parameters"), default = list())
  ui_de <- ui_params$`Differential Expression UI Parameters` %||% list()
  ui_enrich <- ui_params$`Enrichment Analysis UI Parameters` %||% list()
  
  # --- Automatic RUV Results ---
  ruv_results <- study_params$`Automatic RUV Optimization Results` %||% list()
  
  # --- Organism Info ---
  org_info <- study_params$`Organism Information` %||% list()

  # Create a flat list of all extracted parameters
  all_params <- list(
    # Method Details
    missing_values_cutoff = as.numeric(s4_prot_filt$groupwise_percentage_cutoff),
    intensity_cutoff_percentile = as.numeric(s4_pep_filt$peptides_intensity_cutoff_percentile),
    normalisation_method = s4_norm$method,
    qvalue_threshold = as.numeric(s4_qval_filt$qvalue_threshold),
    proteotypic_peptide_only = as.logical(as.numeric(s4_qval_filt$choose_only_proteotypic_peptide)),
    proportion_samples_below_cutoff = as.numeric(s4_pep_filt$peptides_proportion_of_samples_below_cutoff),
    min_peptidoforms_per_protein = as.numeric(s4_pep_prot_filt$peptidoforms_per_protein_cutoff),
    min_peptides_per_sample = as.numeric(s4_sample_filt$peptides_per_sample_cutoff),
    
    # RUV Auto-optimization
    best_percentage = ruv_results$`Best percentage`,
    best_k = ruv_results$`Best k value`,
    num_neg_ctrl = ruv_results$`Control genes`,
    
    # Organism
    organism_name = org_info$`Organism Name`,
    taxon_id = org_info$`Taxon ID`,
    
    # DE - S4
    s4_de_q_val_thresh = as.numeric(s4_de$de_q_val_thresh),
    s4_de_treat_lfc_cutoff = as.numeric(s4_de$treat_lfc_cutoff),
    s4_ebayes_trend = as.logical(s4_de$eBayes_trend),
    s4_ebayes_robust = as.logical(s4_de$eBayes_robust),
    s4_formula_string = s4_de$formula_string,
    
    # DE - UI
    ui_de_q_value_threshold = as.numeric(ui_de$q_value_threshold),
    ui_de_log_fold_change_cutoff = as.numeric(ui_de$log_fold_change_cutoff),
    ui_de_treat_enabled = as.logical(ui_de$treat_enabled),
    
    # Enrichment - UI
    ui_enrichment_up_lfc = as.numeric(ui_enrich$up_log2fc_cutoff),
    ui_enrichment_down_lfc = as.numeric(ui_enrich$down_log2fc_cutoff),
    ui_enrichment_q_value_cutoff = as.numeric(ui_enrich$q_value_cutoff),
    ui_enrichment_database_source = ui_enrich$database_source
  )
...
AnalyzeSamples <- function(design_matrix, study_params) {
  ...
  params_extracted <- ExtractStudyParameters(study_params)
  # Combine base counts/metrics with extracted params
  # Make sure to handle potential NULLs from extraction
...
# --- REWRITTEN ---
# This chunk contains the new robust parser.

# New robust parser for the new study_parameters.txt format
ReadStudyParams <- function(filepath) {
  ...
  for (line in lines) {
    ...
    # Detect main section (e.g., "Basic Information:") - no leading space
    if (grepl(":$", line) && !grepl("^[[:space:]]", line)) {
    ...
    # Detect sub-section (e.g., "Parameters from Final S4 Object:") - has leading space
    if (grepl(":$", line) && grepl("^[[:space:]]+", line)) {
    ...
    # Detect sub-sub-section (e.g., "[globalParameters]")
    if (grepl("^\\[.*\\]$", line)) {
      sub_sub_section <- gsub("\\[|\\]", "", line)
      # Place it under the current main/sub section context
      if (!is.null(sub_section)) {
        params[[main_section]][[sub_section]][[sub_sub_section]] <- list()
      } else if (!is.null(main_section)) {
        params[[main_section]][[sub_sub_section]] <- list()
      }
      next
    }
    
    # Handle contrasts
    if (!is.null(main_section) && main_section == "Contrasts") {
      # Build a character vector of contrasts
      if (is.list(params[[main_section]]) && length(params[[main_section]]) == 0) {
          params[[main_section]] <- c(line)
      } else {
          params[[main_section]] <- c(params[[main_section]], line)
      }
      next
    }

    # Handle key-value pairs (can be : or =)
    if (grepl(":", line) || grepl("=", line)) {
      # Use a more robust split method from stringr
      kv_mat <- stringr::str_split_fixed(line, "[=:]", 2)
      key <- trimws(kv_mat[1, 1])
      value <- trimws(kv_mat[1, 2])
        
      # Clean bullet points from keys
      key <- sub("^[•[:space:]]+", "", key)

      # Assign value to the correct nested level, only if key is valid
      if(nchar(key) > 0) {
          if (!is.null(sub_sub_section)) {
            params[[main_section]][[sub_section]][[sub_sub_section]][[key]] <- value
          } else if (!is.null(sub_section)) {
...
Main <- function() { # Removed suffix param
  ...
  # Run analysis using REWRITTEN AnalyzeSamples
