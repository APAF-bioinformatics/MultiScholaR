...
conflicts_prefer(vroom::col_guess)
...
```{r 4 Import DIA-NN Data}
# Convert your EList object to DIA-NN format
...
```{r 5 Process FASTA and Annotate Proteins}
...
```{r 6 Protein ID Conversion and Uniprot Lookup}
...
```{r 8 Design Matrix Output Read In - optional}
...
```{r 9 Peptide Data S4 Object Creation & Annotation API Lookup}
peptide_data <- new(
...
```{r 11 Filter Peptides on q Value and Proteotypic Peptide Match}
...
```{r 12 Roll-up Precursor to Peptide}
...
```{r 14 Cyclic loess normalisation}
...
```{r 15 RUV-III-c paramater tuning and normalisation}
...
savePlot( 
  QC_composite_figure@cancor_plots$cancor_plot_peptides_ruvIIIc_group,
  project_dirs[[paste0(omic_type, "_", experiment_label)]]$peptide_qc_dir,
  "peptide_cancor_plot_ruvIIIc_by_group"
)
...
```{r 17 Limpa imputation and protein rollup}
...
```{r 18 Filter Proteins on Peptide Number}
...
```{r 19 Sample Pearson Correlation Filtering}
...
ruv_normalised_for_de_analysis_obj <- filterSamplesByProteinCorrelationThreshold(
  removed_proteins_with_insufficient_evidence,
...
# Update tracking for downstream analysis
updateProteinFiltering(
  data = ruv_normalised_for_de_analysis_obj@protein_quant_table,
...
```{r 20 Output Files For Audit Trail}
ruv_normalised_for_de_analysis_obj <- ruv_normalised_for_de_analysis_obj
...
if(!is.null( project_dirs[[paste0("integration_", experiment_label)]]$multiomic_inputs_dir)) {
  saveRDS(
    ruv_normalised_for_de_analysis_obj, 
    file.path(
...
```{r 21 Composite QC Figure Generation}
...
```{r 23 Output DE Analysis Results}
...
```{r 24 STRING DB Functional Enrichment Analysis}
...
```{r 25 Plot the StringDB Results}
...
```{r 26 Threshold-Based Enrichment Analysis, verbose = FALSE}
...
```{r 27 Save Workflow Arguments and Study SUmmary} 
...
```{r 28 Copy Files to Publication Directory}
