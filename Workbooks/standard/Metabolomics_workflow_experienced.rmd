---
title: "Metabolomics Analysis Workflow"
version: "1.0"
...
# Reuse the existing function if suitable
experiment_label <- "Metabolomics_Experiment_1" # NB this will affect directory structure and name
multisetupAndShowDirectories(omic_type = "metabolomics"
...
metabolomics_data_file <- "metabolite_intensities.tsv" # Example: wide format (rows=metabolites, cols=samples) or long format
metadata_file <- "metabolite_metadata.tsv" # Optional: File with ChEBI IDs, names, formulas etc. if not in main data file
config_file <- "metabolomics_config.ini" # Configuration file for parameters

# --- Configuration Loading ---
# Reuse if applicable, otherwise load parameters manually
# config_list <- readConfigFile(file = file.path(source_dir, config_file))
# Example manual parameters if config file not used:
config_list <- list(
  globalParameters = list(
    metabolomics_input_file = metabolomics_data_file,
    metadata_input_file = metadata_file,
    sample_id_column = "SampleID", # Column name for sample IDs in data/design matrix
    metabolite_id_column = "MetaboliteID", # Column name for unique metabolite IDs in data/metadata
    intensity_column = "Intensity", # Column name for intensity values (if long format)
    feature_name_column = "MetaboliteName", # Column for display names
    annotation_source = "ChEBI", # or "KEGG", "PubChem", "HMDB"
    internal_standard_pattern = "ITSD", # Pattern to identify internal standards (e.g., in MetaboliteID or a dedicated column)
    organism_kegg_code = "hsa" # Example: Homo sapiens KEGG code (used for pathway mapping)
  ),
  qcParameters = list(
    min_fraction_detected_per_group = 0.7, # Minimum fraction of non-missing values within a group to keep a feature
    min_groups_detected = 1, # Minimum number of groups meeting the fraction threshold
    intensity_quantile_threshold = 0.01 # Quantile for low-intensity filtering (if used)
  ),
  normalizationParameters = list(
    is_normalization_method = "sum", # Method to aggregate IS intensity ('sum', 'mean', 'median')
    post_is_normalization = "log2" # Optional: 'log2', 'PQN', 'median', 'none'
  ),
  ruvParameters = list(
    perform_ruv = TRUE,
    k_value = NULL, # Set manually or determine using cancorplot
    use_internal_standards_as_neg_ctrl = TRUE, # Use IS identified by pattern as negative controls for RUV
    percentage_as_neg_ctrl = 10, # Alternative: Use % lowest variance features if not using IS
    ruv_grouping_variable = "group" # Column in design matrix for RUV factor estimation
  ),
  correlationFilterParameters = list(
    perform_correlation_filter = TRUE,
    min_pearson_correlation_threshold = 0.6, # Threshold for filtering samples based on within-group correlation
    correlation_group = "group" # Column in design matrix for grouping samples
  )
)

log_info("Configuration parameters loaded/defined.")

# --- Data Loading ---
# Assuming wide format initially: rows=metabolites, cols=samples, first col = metabolite ID
data_raw <- vroom::vroom(
  file.path(data_dir, "metabolomics", config_list$globalParameters$metabolomics_input_file),
  delim = "\t", # Adjust delimiter if needed
  col_types = cols(.default = col_double(), !!config_list$globalParameters$metabolite_id_column := col_character()) # Ensure ID is char, others numeric
)

# --- Metadata Loading (Optional) ---
# Load if metabolite annotations are in a separate file
if (!is.null(config_list$globalParameters$metadata_input_file)) {
  feature_metadata_raw <- vroom::vroom(
    file.path(data_dir, "annotations", config_list$globalParameters$metadata_input_file),
    delim = "\t" # Adjust delimiter if needed
  )
  log_info("Metabolite metadata loaded from: {config_list$globalParameters$metadata_input_file}")

  # Basic check: Ensure metabolite ID column exists in metadata
  if (!config_list$globalParameters$metabolite_id_column %in% names(feature_metadata_raw)) {
      log_error("Metabolite ID column '{config_list$globalParameters$metabolite_id_column}' not found in metadata file.")
      stop("Metadata file missing required ID column.")
  }

} else {
  # If no separate metadata file, assume required info is in the main data file's non-numeric columns
  # Or generate minimal metadata from the data_raw file
  feature_metadata_raw <- data_raw |>
      select(where(\\(x) !is.numeric(x))) # Select non-numeric columns as potential metadata
  
  # Ensure the primary ID column is present
  if (!config_list$globalParameters$metabolite_id_column %in% names(feature_metadata_raw)) {
      log_error("Metabolite ID column '{config_list$globalParameters$metabolite_id_column}' not found in data file.")
      stop("Data file missing required ID column.")
  }
    
  log_info("Metadata extracted from data file columns.")
}

# Separate quantitative data from metadata if combined initially
quantitative_data_raw <- data_raw |>
    select(!!config_list$globalParameters$metabolite_id_column, where(is.numeric)) |>
    tibble::column_to_rownames(var = config_list$globalParameters$metabolite_id_column) |>
    as.matrix()

log_info("Raw quantitative data dimensions: {nrow(quantitative_data_raw)} metabolites x {ncol(quantitative_data_raw)} samples")
log_info("Raw feature metadata dimensions: {nrow(feature_metadata_raw)} metabolites x {ncol(feature_metadata_raw)} annotations")

# --- Placeholder for Initial Data Cleaning ---
# e.g., Remove empty rows/columns, handle initial format conversions if needed
# data_cln <- initialDataClean(quantitative_data_raw, feature_metadata_raw)
# For now, assume raw data is relatively clean
data_cln <- quantitative_data_raw
feature_metadata_cln <- feature_metadata_raw |>
    # Ensure the metabolite ID column used for joining/rownames is consistent
    dplyr::rename(rowname_id = !!config_list$globalParameters$metabolite_id_column) |>
    # Remove potential duplicate metadata rows if metabolite IDs are repeated
    dplyr::distinct(rowname_id, .keep_all = TRUE) |>
    # Ensure metadata rows match the quantitative data rows
    dplyr::filter(rowname_id %in% rownames(data_cln))

# Align metadata rows exactly with quantitative data rows
feature_metadata_cln <- feature_metadata_cln[match(rownames(data_cln), feature_metadata_cln$rowname_id), ]

log_info("Cleaned quantitative data dimensions: {nrow(data_cln)} x {ncol(data_cln)}")
log_info("Cleaned feature metadata dimensions: {nrow(feature_metadata_cln)} x {ncol(feature_metadata_cln)}")
...
# Reuse the Shiny applet if available and compatible
# Need to ensure the applet generates columns expected by downstream functions (e.g., 'Run', 'group', 'replicates')
# The applet likely needs the sample names (column names from data_cln) as input.
sample_names <- colnames(data_cln)

if (exists("design_matrix", envir = .GlobalEnv)) {
  log_info("Design matrix already exists in environment. Skipping applet.")
} else {
  log_info("Launching Design Matrix Applet...")
  # RunApplet("designMatrix", sample_list = sample_names) # Pass sample names to the applet
  # --- Applet Placeholder ---
  # If RunApplet doesn't exist or isn't suitable, provide manual creation example:
  log_warn("RunApplet function not found or not run. Creating a placeholder design matrix.")
  design_matrix <- tibble::tibble(
      Run = sample_names,
      # --- !!! Manually define your groups and replicates here !!! ---
      group = rep(c("Control", "Treatment"), each = ncol(data_cln)/2), # Example grouping
      replicates = paste0("BioRep", 1:ncol(data_cln)) # Example replicate info
      # Add other relevant columns (e.g., batch, timepoint)
  )
  # --- End Placeholder ---
  log_info("Design matrix created.")
  # Optional: Save the newly created design matrix
  # write.table(design_matrix, file.path(source_dir, "design_matrix_metabolomics.tab"), sep = "\t", row.names = FALSE, quote = FALSE)
}

# Validate design matrix against data
if (!all(colnames(data_cln) %in% design_matrix$Run)) {
    log_error("Mismatch between sample names in data columns and 'Run' column in design matrix.")
    stop("Sample ID mismatch. Check data loading and design matrix creation.")
}
# Ensure design matrix only contains samples present in the data
design_matrix <- design_matrix |> filter(Run %in% colnames(data_cln))
# Reorder data columns to match design matrix order (important for matrix operations)
data_cln <- data_cln[, design_matrix$Run]

log_info("Design matrix dimensions: {nrow(design_matrix)} samples x {ncol(design_matrix)} factors")
...
# Ensure column names in design_matrix match expected IDs for the S4 object
# The constructor/base class might expect specific names like 'sample_id', 'group_id'
# Map columns from design_matrix to the expected names if necessary
sample_data_for_s4 <- design_matrix |>
  dplyr::rename(
    # Rename columns if the S4 object or its methods expect specific names
    # e.g., sample_id = !!config_list$globalParameters$sample_id_column, # Already 'Run' in example
    group = group, # Assuming 'group' column exists
    technical_replicate_id = replicates # Assuming 'replicates' column exists
  ) |>
  # Ensure the sample ID column becomes the rownames for colData
  tibble::column_to_rownames(var = "Run") # Use the original sample ID column name

# Create the object using the constructor defined earlier (or adapt as needed)
metabolite_data_obj <- createMetabolomicsData(
  quantitative_data = data_cln,
  feature_metadata = feature_metadata_annotated,
  sample_metadata = sample_data_for_s4,
  metabolite_id_column = "rowname_id", # The consistent ID column in feature_metadata
  sample_id_column = "Run", # The consistent ID column in sample_metadata (used for rownames)
  config_list = config_list
)

# Add initial processing step to log
metabolite_data_obj@processing_log$s4_creation <- list(
    time = Sys.time(),
    dims = dim(metabolite_data_obj),
    description = "Created MetabolomicsQuantitativeData object from raw data."
)

log_info("MetabolomicsQuantitativeData object created.")
print(metabolite_data_obj)
...
```''', instructions='Create skeleton R Markdown workflow for metabolomics, mirroring proteomics workflow up to RUV step.'))
```