...
# mod_lipid_import.R
# ============================================================================
# Purpose: Lipidomics data import Shiny module
#
# This module provides multi-assay file import with vendor format detection
# and dynamic column mapping for lipidomics data.
# ============================================================================

#' @title Lipidomics Import Module
#' @description A Shiny module for importing lipidomics data with vendor format
  ...
  shiny::tagList(
    ...
    shiny::fluidRow(
      shiny::column(
        12,
        shiny::wellPanel(
          shiny::h3("Lipidomics Data Import"),
          shiny::fluidRow(
...
mod_lipid_import_server <- function(id, workflow_data, experiment_paths, volumes = NULL) {
  message("--- Entering mod_lipid_import_server ---")
  message(sprintf("   mod_lipid_import_server: volumes param is NULL = %s", is.null(volumes)))

  shiny::moduleServer(id, function(input, output, session) {
    message("   mod_lipid_import_server: Inside moduleServer function")

    # Check if shinyFiles is available
    use_shiny_files <- requireNamespace("shinyFiles", quietly = TRUE)
    message(sprintf("   mod_lipid_import_server: shinyFiles available = %s", use_shiny_files))
    ...
    if (use_shiny_files) {
      if (is.null(volumes)) {
        message("   mod_lipid_import_server: volumes is NULL, creating from getVolumes()")
      ...
      if (length(volumes) > 0) {
        ...
        message("   mod_lipid_import_server: WARNING - volumes is empty!")
      }
      ...
      tryCatch(
        {
          ...
          # Detect format
          format_info <- detectLipidomicsFormat(
            headers = headers,
    ...
    # Lipid ID validation status
    output$lipid_id_status <- shiny::renderUI({
      shiny::req(local_data$assay1_data, input$lipid_id_col)

      if (input$lipid_id_col %in% names(local_data$assay1_data)) {
        n_unique <- length(unique(local_data$assay1_data[[input$lipid_id_col]]))
        shiny::tags$span(
    ...
    # Custom mode: Lipid ID validation status
    output$lipid_id_status_custom <- shiny::renderUI({
      shiny::req(local_data$assay1_data)

      col_name <- input$lipid_id_col_custom
    ...
    # Helper to get the effective lipid ID column (dropdown or custom)
    get_lipid_id_col <- shiny::reactive({
    ...
    get_sample_columns <- shiny::reactive({
      ...
      if (!is.null(local_data$assay1_import_result)) {
        ...
        if (isTRUE(input$exclude_norm)) {
          exclusion_pattern <- "_norm(ali[sz]ed|allized)?$|normalized"
          sample_cols <- sample_cols[!grepl(exclusion_pattern, sample_cols, ignore.case = TRUE)]
        }
      ...
      sample_cols <- names(local_data$assay1_data)[sapply(local_data$assay1_data, is.numeric)]

      # Apply exclusion filter if enabled
      if (isTRUE(input$exclude_norm)) {
        # Pattern to match: _Norm, _Normalised, _normalized, _normallized, Normalized
        # Case-insensitive due to ignore.case = TRUE
        exclusion_pattern <- "_norm(ali[sz]ed|allized)?$|normalized"

        # Keep columns that DO NOT match the pattern
        sample_cols <- sample_cols[!grepl(exclusion_pattern, sample_cols, ignore.case = TRUE)]
      }
    ...
    shiny::observeEvent(input$process_import, {
      ...
      if (isTRUE(input$capture_checkpoints)) {
        options(multischolar.checkpoint_omics_layer = "lipidomics")
      ...
      tryCatch(
        {
          ...
          if (isTRUE(input$sanitize_names)) {
            ...
            # Capture original sample names to use for matching across ALL assays
            original_sample_cols <- sample_cols
            cleaned_sample_cols <- janitor::make_clean_names(original_sample_cols)

            # Sanitize sample columns in assay_list
            assay_list <- purrr::map(assay_list, function(assay_df) {
              all_cols <- names(assay_df)

              # Match columns in THIS assay that were in the original sample list
              match_indices <- match(original_sample_cols, all_cols)

              # Only update columns that actually exist in this assay
              valid_matches <- !is.na(match_indices)
              if (any(valid_matches)) {
                names(assay_df)[match_indices[valid_matches]] <- cleaned_sample_cols[valid_matches]
              }

              assay_df
            })

            # Update the global sample_cols list to the cleaned version
            sample_cols <- cleaned_sample_cols
          ...
          if (!is.null(workflow_data$state_manager)) {
            workflow_data$state_manager$setWorkflowType("lipidomics_standard")
            logger::log_info("Workflow type set to: lipidomics_standard")
          }
          ...
          workflow_data$processing_log$setup_import <- list(
            ...
            n_lipids = sapply(assay_list, function(a) {
              if (lipid_col %in% names(a)) {
                length(unique(a[[lipid_col]]))
              } else {
                nrow(a)
              }
            }),
          ...
          shiny::removeNotification("lipid_import_working")
