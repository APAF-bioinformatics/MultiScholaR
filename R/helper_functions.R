...
#' @export
cmriFormatter <- function(record) {
    sprintf("CMRI Bioinformatics %s [%s] | %s", record$levelname, record$timestamp, record$msg)
}
...
loadDependencies <- function(verbose = TRUE) {
    ...
    required_packages <- c(
        ...
        "dplyr", "gtools", "shiny", "DT", "gh", "openxlsx", "logger",
        # Additional packages
...
setupAndShowDirectories <- function(base_dir = here::here(), label = NULL, force = FALSE) {
    ...
    paths <- list(
        ...
        special = list(
            ...
            time = file.path(
                base_dir, "results", proteomics_dirname, "publication_graphs",
...
copyToResultsSummary <- function(contrasts_tbl, label = NULL, force = FALSE, current_rmd = NULL) {
    ...
    if (dir.exists(results_summary_dir)) {
        ...
        backup_dirname <- if (!is.null(label)) {
            paste0(
                "proteomics_", substr(label, 1, 30), "_backup_",
                ...
                if (!is.null(file_spec$type) && file_spec$type == "object") {
                    ...
                    if (copy_success) {
                        ...
                        if (length(files_to_copy) > 0) {
                            ...
                            if (length(source_files) != length(dest_files)) {
                                copy_success <- FALSE
                                error_msg <- sprintf(
                                    "File count mismatch: %d source files, %d destination files",
                    ...
                    if (!file.copy(from = source_path, to = dest_path, overwrite = TRUE)) {
                        ...
                        if (source_size != dest_size) {
                            copy_success <- FALSE
                            error_msg <- sprintf(
                                "File size mismatch: source=%d bytes, dest=%d bytes",
