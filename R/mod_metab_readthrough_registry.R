...
.METAB_DESIGN_PARAMETER_NAMES <- c(
...
metabArtifactReadthroughAdapter <- function() {
    newArtifactStageReadthroughAdapter(
        descriptor = artifactMetabolomicsWorkflowDescriptor(),
        owner_label = "metabolomics",
        default_omic_label = "metabolomics",
        abort_fn = metabArtifactAbort,
        conditions = c(
            corrupt_manifest = "multischolar_corrupt_metabolomics_manifest",
            invalid_context = "multischolar_invalid_metabolomics_resume_context",
            descriptor_pin_missing = "multischolar_metabolomics_descriptor_pin_missing",
            descriptor_pin_mismatch = "multischolar_metabolomics_descriptor_pin_mismatch",
            registry_ref_mismatch = "multischolar_metabolomics_registry_ref_mismatch",
            missing_committed_stage = "multischolar_missing_metabolomics_stage",
            invalid_run_parameters = "multischolar_invalid_metabolomics_run_parameters",
            missing_current_state = "multischolar_missing_metabolomics_state",
            incomplete_contract = "multischolar_incomplete_metabolomics_readthrough",
            mixed_generation = "multischolar_mixed_metabolomics_generation",
            invalid_optional_payload = "multischolar_invalid_metabolomics_payload",
            invalid_contrasts = "multischolar_invalid_metabolomics_contrasts",
            invalid_column_mapping = "multischolar_invalid_metabolomics_mapping"
        )
    )
}

metabArtifactProbeManifest <- function(experiment_paths, experiment_label) {
...
createMetabResumeContext <- function(
    experiment_paths,
    experiment_label,
    storage_policy = NULL
) {
...
metabArtifactProjectPresent <- function(
    experiment_paths,
    experiment_label,
    storage_policy = NULL
) {
    ...
    tryCatch(
        isTRUE(metabArtifactProbeManifest(
            experiment_paths,
            experiment_label
        )$found),
        error = \(...) TRUE
...
metabArtifactRunHasParameters <- function(
    resources,
    run_id,
    parameter_names
) {
...
metabArtifactStageRoles <- function(stage_id, parameters) {
    ...
    if (stage_id == "import") {
        ...
        return(c(assay_roles, .METAB_IMPORT_FIXED_ROLES))
    }
...
resolveMetabCommittedStage <- function(
    resources,
    stage_id,
    parameter_names,
    run_id = NULL,
    ...
    if (nrow(runs) > 0L) {
        for (index in rev(seq_len(nrow(runs)))) {
            ...
            if (!metabArtifactRunHasParameters(
                resources,
                run$run_id,
                parameter_names
            )) next
            ...
            roles <- metabArtifactStageRoles(stage_id, parameters)
    ...
    artifactStageReadthroughAbort(
        ...
        sprintf("metabolomics stage '%s' has no complete artifact set", stage_id),
        stage_id = stage_id,
        requested_run_id = run_id
    )
}

collectMetabResumeEvidence <- function(
    context,
    ...
    design <- resolveMetabCommittedStage(
        resources,
        "design",
        .METAB_DESIGN_PARAMETER_NAMES,
    ...
    if (!workflowCapabilityScalarString(parent_run_id)) {
        artifactStageReadthroughAbort(
            adapter,
            "incomplete_contract",
            "metabolomics design does not declare one parent import run"
        )
    }
    import <- resolveMetabCommittedStage(
        resources,
        "import",
        .METAB_IMPORT_PARAMETER_NAMES,
