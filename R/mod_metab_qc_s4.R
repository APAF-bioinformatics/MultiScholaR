...
mod_metab_qc_s4_server <- function(id, workflow_data, omic_type, experiment_label) {
    shiny::moduleServer(id, function(input, output, session) {
        ...
        output$data_summary <- shiny::renderUI({
            ...
            total_metabolites <- sum(sapply(assay_list, function(a) {
                if (current_s4@metabolite_id_column %in% names(a)) {
                    length(unique(a[[current_s4@metabolite_id_column]]))
                } else {
                    nrow(a)
                }
            }))
            ...
            shiny::tagList(
                shiny::tags$table(
                    ...
                    , shiny::tags$tbody(
                        ...
                        , shiny::tags$tr(
                            shiny::tags$td(shiny::strong("Total Metabolites:"))
                            , shiny::tags$td(format(total_metabolites, big.mark = ","))
                        )
                        , shiny::tags$tr(
                        ...
                        , shiny::tags$tr(
                            shiny::tags$td(shiny::strong("Metabolite ID Column:"))
                            , shiny::tags$td(current_s4@metabolite_id_column)
                        )
                        , shiny::tags$tr(
        ...
        output$assay_stats_table <- DT::renderDT({
            ...
            stats_df <- data.frame(
                ...
                , Metabolites = sapply(assay_list, function(a) {
                    if (metabolite_id_col %in% names(a)) {
                        length(unique(a[[metabolite_id_col]]))
                    } else {
                        nrow(a)
                    }
                })
        ...
        shiny::observeEvent(input$finalize_qc, {
            ...
            tryCatch({
                ...
                result_text <- paste(
                    ...
                    , "========================"
                    , ""
                    , sprintf("Total metabolites retained: %d", total_metabolites)
                ...
                logger::log_info("Metabolomics QC finalized successfully")
