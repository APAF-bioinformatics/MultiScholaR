...
mod_lipid_qc_itsd_ui <- function(id) {
    ns <- shiny::NS(id)
        ...
        , shiny::fluidRow(
            shiny::column(4
                , shiny::wellPanel(
                    ...
                    , shiny::helpText("Regular expression to identify internal standards in lipid IDs. Leave empty to use pattern from S4 object.")
...
mod_lipid_qc_itsd_server <- function(id, workflow_data, omic_type, experiment_label) {
    shiny::moduleServer(id, function(input, output, session) {
        ...
        shiny::observeEvent(input$analyze_is, {
            ...
            tryCatch({
                ...
                # Try lipid_id_col first, then annotation_id_col, then common MS-DIAL columns
                ...
                for (i in seq_along(assay_list)) {
                    ...
                    if (nrow(is_met) > 0) {
                        ...
                        id_col_for_filter <- if (!is.null(used_col)) used_col else lipid_id_col
                        quant_info <- getLipidQuantData(assay_data)
                ...
                if (length(metrics_list) == 0) {
                    ...
                    stop(paste0(
                        ...
                        "\nTip: Check if your data has a column with lipid names (e.g., 'Lipid name')"
                    ))
                }
