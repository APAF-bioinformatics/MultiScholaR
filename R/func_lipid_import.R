...
# func_lipid_import.R
# ============================================================================
# Purpose: Lipidomics data import functions
#
# This file contains functions for importing lipidomics data from various
# platforms and formats. Functions in this file are used by lipidomics
# import modules and related workflows.
#
# Functions to extract here:
# - createLipidomicsAssayData(): Creates LipidomicsAssayData S4 object
# - getLipidQuantData(): Extracts quantitative data from assay tibbles
# - Additional lipidomics import helper functions
#
# Dependencies:
# - dplyr, tidyr
...
# ----------------------------------------------------------------------------
#' @title Detect Lipidomics Data Format
#' @description Auto-detect the vendor format of lipidomics data based on
...
#'
#' @export
detectLipidomicsFormat <- function(headers, filename = NULL) {
    ...
    ls_found <- sum(ls_markers %in% headers_lower)
    ls_score <- ls_found / min(length(ls_markers), 7)

    # LipidSearch uses "LipidName" (no space) vs MS-DIAL "Lipid name" (space)
    # Strong differentiator
    if ("lipidclass" %in% headers_lower && "lipidname" %in% headers_lower) {
        ls_score <- min(1.0, ls_score + 0.3)
    }

    if (grepl("lipidsearch|alignment", filename_lower)) {
        ls_score <- min(1.0, ls_score + 0.1)
    }

    # Determine best match
    scores <- c(
        msdial = msdial_score,
        lipidsearch = ls_score,
...
#'   - `lipid_id`: Primary identifier column
#'   - `annotation`: Lipid name/annotation column
...
#'
#' @export
getLipidomicsColumnDefaults <- function(format) {
    defaults <- switch(tolower(format),
        "msdial" = list(
            # Support both old format (Alignment ID) and new format (Peak ID)
            lipid_id = c("Peak ID", "peak id", "Alignment ID", "alignment id"),
            annotation = c("Name", "name", "Lipid name", "lipid name"),
        ...
        "xcms" = list(
            lipid_id = c("featureid", "featureId", "FeatureID"),
            annotation = NULL,
            rt = c("rt", "RT"),
            mz = c("mz", "MZ", "m/z"),
            adduct = NULL,
        ...
        "compound_discoverer" = list(
            lipid_id = c("Compound Name", "compound name"),
        ...
        "lipidsearch" = list(
            lipid_id = c("LipidName", "lipidname", "Lipidion", "lipidion"),
            annotation = c("LipidClass", "lipidclass", "Class", "class"),
            rt = c("BaseRt", "basert", "RT", "rt"),
            mz = c("CalcMz", "calcmz", "mz", "MZ"),
            adduct = c("Adduct", "adduct", "IonType", "iontype"),
            is_pattern = "^IS_|ISTD|Internal|_IS"
        ),
        "custom" = list(
            lipid_id = NULL,
            annotation = NULL,
...
#' @param lipid_id_column Name of the lipid ID column.
...
validateColumnMapping <- function(data, lipid_id_column, sample_columns) {
    ...
    # Check lipid ID column
    if (is.null(lipid_id_column) || !nzchar(lipid_id_column)) {
        errors <- c(errors, "Lipid ID column is not specified")
    } else if (!lipid_id_column %in% names(data)) {
        errors <- c(errors, paste("Lipid ID column not found:", lipid_id_column))
    }
    ...
    summary_stats <- list()
    if (length(errors) == 0) {
        summary_stats$n_lipids <- length(unique(data[[lipid_id_column]]))
        summary_stats$n_samples <- length(sample_columns)

        # Check for duplicates
        n_duplicates <- sum(duplicated(data[[lipid_id_column]]))
        if (n_duplicates > 0) {
            warnings <- c(warnings, sprintf("%d duplicate lipid IDs detected", n_duplicates))
        }
