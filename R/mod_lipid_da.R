...
mod_lipid_da_server <- function(id, workflow_data, experiment_paths, omic_type, experiment_label) {
    shiny::moduleServer(id, function(input, output, session) {
            ...
            tryCatch(
                {
                    ...
                    if (!is.null(results$da_lipids_long)) {
                        contrast_choices <- unique(results$da_lipids_long$friendly_name)
                        if (length(contrast_choices) == 0) {
                            contrast_choices <- unique(results$da_lipids_long$comparison)
                        }
                        ...
                        assay_choices <- c("Combined", unique(results$da_lipids_long$assay))
                        ...
                        table_assay_choices <- c("All", unique(results$da_lipids_long$assay))
            ...
            tryCatch(
                {
                    ...
                    widget <- generateLipidDAVolcanoPlotGlimma(
        ...
        output$da_summary_stats <- shiny::renderPrint({
            ...
            results <- da_data$da_results_list$da_lipids_long
        ...
        output$download_da_results <- shiny::downloadHandler(
            filename = function() {
                paste0("lipidomics_da_results_", Sys.Date(), ".csv")
            },
