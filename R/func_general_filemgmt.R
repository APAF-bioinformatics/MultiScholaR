...
readConfigFile <- function( file=file.path(source_dir, "config.ini")) {
  ...
  message(sprintf("DEBUG66: read.config() completed, config_list names = %s", 
      paste(names(config_list), collapse = ", ")))
  message(sprintf("DEBUG66: config_list class = %s, length = %d", 
  ...
  if("plotRle" %in% names(config_list)) {
   ...
   if("deAnalysisParameters" %in% names(config_list)) {
    ...
    message(sprintf("DEBUG66: deAnalysisParameters keys = %s", 
...
# ============================================================================
# Metabolomics S4 Parameter Extraction Helper
# ============================================================================
...
extractMetabS4Params <- function(metab_s4) {
    ...
    tryCatch({
        ...
        cat(sprintf("METAB PARAMS: Found %d assays: %s\n",
    ...
    tryCatch({
        ...
        cat(sprintf("METAB PARAMS: Log transformation applied: %s\n",
    ...
    tryCatch({
        ...
        cat(sprintf("METAB PARAMS: Normalization method: %s\n",
    ...
    tryCatch({
        if (!is.null(s4_args$ruv_number_k) || !is.null(s4_args$ruv_grouping_variable)) {
            ...
            if (is.list(s4_args$ruv_number_k)) {
                ...
                cat(sprintf("METAB PARAMS: RUV-III applied per-assay with k values: %s\n",
                ...
                cat(sprintf("METAB PARAMS: RUV-III applied with k=%s\n",
    ...
    cat(sprintf("METAB PARAMS: Extraction complete. Found %d parameter groups\n", length(result)))
    return(result)
}
...
createWorkflowArgsFromConfig <- function(workflow_name, description = "", 
    ...
    git_info <- tryCatch({
        ...
        if (!is.na(pkg_path) && dir.exists(file.path(pkg_path, ".git"))) {
            ...
            # Get current branch
            branch <- tryCatch({
                result <- system2("git", args = c("-C", pkg_path, "branch", "--show-current"),
            ...
            commit_sha <- tryCatch({
                result <- system2("git", args = c("-C", pkg_path, "rev-parse", "HEAD"),
                                  stdout = TRUE, stderr = FALSE)
                if (length(result) > 0 && nzchar(result[1])) result[1] else NA_character_
            }, error = function(e) NA_character_)
            ...
            timestamp <- tryCatch({
                result <- system2("git", args = c("-C", pkg_path, "log", "-1", "--format=%cI"),
                                  stdout = TRUE, stderr = FALSE)
                if (length(result) > 0 && nzchar(result[1])) result[1] else NA_character_
            }, error = function(e) NA_character_)
            ...
            repo_name <- tryCatch({
                remote <- system2("git", args = c("-C", pkg_path, "remote", "get-url", "origin"),
                                  stdout = TRUE, stderr = FALSE)
