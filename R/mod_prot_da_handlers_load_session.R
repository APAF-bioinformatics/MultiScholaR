    ...
    tryCatch(
      {
        ...
        shiny::withProgress(message = "Loading filtered session data...", value = 0, {
          ...
          state_snapshot <- restoreWorkflowStateFromSession(
            workflow_data$state_manager,
            session_data
          )
          r6_complete_states <- state_snapshot$r6_complete_states
          r6_state_history <- state_snapshot$r6_state_history
          restored_state_name <- state_snapshot$r6_current_state_name
