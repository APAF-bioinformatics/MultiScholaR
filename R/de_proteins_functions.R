...
removeRowsWithMissingValuesPercentHelper <- function(
  ...
  abundance_long <- input_table |>
    tidyr::pivot_longer(
      cols = !all_of(cols) # Use !all_of() with the cols string directly
    ...
    dplyr::mutate(!!sym(temporary_abundance_column) := dplyr::case_when(
    ...
    dplyr::left_join(
      ...
      by = sample_id_col_name_string
    ) # Join using the explicit string name
  ...
  print(sprintf(
    "      removeRowsWithMissingValuesPercentHelper: Calculated min_protein_intensity_threshold = %g (percentile %g%%)",
...
proteinTechRepCorrelationHelper <- function(
  ...
  frozen_protein_matrix_tech_rep <- data_matrix |>
    ...
    mutate(log2_intensity = as.numeric(log2_intensity)) |>
