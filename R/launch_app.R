  ...
  switch(omic_type,
    "proteomics" = {
      ...
      if (applet_type == "designMatrix") {
        logger::log_info(paste("Launching Design Matrix applet for PROTEOMICS. Using source_dir:", source_dir))
        ...
        server <- function(input, output, session) {
          ...
          module_results_rv <- designMatrixBuilderServer(
            "builder",
            data_tbl = shiny::reactive(data_tbl),
            config_list = shiny::reactive(config_list)
          )

          shiny::observeEvent(module_results_rv(), {
            results <- module_results_rv()
            shiny::req(results)

            logger::log_info("Standalone Applet: Saving results and closing.")
            
            shiny::showModal(shiny::modalDialog(
              title = "Saving Data",
              shiny::p("Saving results and closing applet..."),
            ...
            logger::log_info("Results assigned to both parent frame and global environment.")
            ...
            tryCatch({
              ...
              if (!is.null(results$contrasts_tbl) && nrow(results$contrasts_tbl) > 0) {
                ...
                logger::log_error(paste("Error saving files in standalone applet:", e$message))
            })
            
            shiny::removeModal()
            shiny::stopApp(results)
          })
        }
    ...
    { # Default for the omic_type switch
      logger::log_error(paste("Unsupported omic_type provided to RunApplet:", omic_type))
