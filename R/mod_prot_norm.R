...
mod_prot_norm_server <- function(id, workflow_data, experiment_paths, omic_type, experiment_label, selected_tab = NULL) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    generatePreNormalizationQc <- function() {
      ...
      message("Step 1: Getting correct S4 object (target: protein_replicate_filtered)...")
      ...
      # FIX: Explicitly try to get the 'protein_replicate_filtered' state
      # This ensures we plot the RAW data, not the current (possibly normalized) data
      current_s4 <- NULL
      target_state <- "protein_replicate_filtered"

      # 1. Try fetching specific state from manager
      tryCatch(
        {
          message(sprintf("Attempting to retrieve state: '%s'", target_state))
          current_s4 <- workflow_data$state_manager$getState(target_state)
        },
        error = function(e) {
          message(sprintf("Could not retrieve specific state '%s': %s", target_state, e$message))
        }
      )

      # 2. If not found, check if CURRENT state IS the target state
      if (is.null(current_s4)) {
        current_state_name <- workflow_data$state_manager$current_state
        message(sprintf("Target state not found directly. Current state is: '%s'", current_state_name))

        if (current_state_name == target_state) {
          message("Current state matches target. Using current state object.")
          current_s4 <- workflow_data$state_manager$getState(current_state_name)
        } else {
          message("WARNING: Could not find 'protein_replicate_filtered' state. Plots might be inaccurate if using advanced state.")
          # Fallback (risky): Use current state?
          # If we are in 'normalized' state, using it for 'pre-norm' plots is WRONG.
          # Better to fail or warn?
          # For now, we will fallback but log a heavy warning.
          current_s4 <- workflow_data$state_manager$getState(current_state_name)
        }
      }

      if (!is.null(current_s4)) {
        message(sprintf("S4 object retrieved. Class: %s", class(current_s4)))
      } else {
        stop("No S4 object available for Pre-Normalization QC plot generation")
      }
      ...
      shiny::observeEvent(selected_tab(),
        {
          ...
          if (!is.null(selected_tab()) && selected_tab() == "normalization") {
            ...
            if (!is.null(workflow_data$state_manager)) {
              ...
              # Auto-trigger logic
              # Trigger if:
              # 1. We are in the initial 'protein_replicate_filtered' state AND plots haven't been generated
              # 2. OR we are in ANY advanced valid state AND plots haven't been generated yet (e.g. reload or missed trigger)
              ...
              # Case 2: Advanced state but plots missing (Fix for "Plot not generated yet")
              else if (current_state %in% valid_states_for_norm_tab && !norm_data$pre_norm_qc_generated) {
                message("*** DETECTED ADVANCED STATE WITH MISSING INITIAL PLOTS - FORCING GENERATION ***")
                should_trigger <- TRUE
              }
              ...
              if (should_trigger) {
                ...
                message(sprintf(
                  "State is '%s' and pre_norm_qc_generated is %s. No trigger needed.",
                  current_state, norm_data$pre_norm_qc_generated
                ))
              } else {
