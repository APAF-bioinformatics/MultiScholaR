    ...
    shiny::moduleServer(id, function(input, output, session) {
        ...
        initial_state <- shiny::reactive({
            ...
            if (!is.null(col_map$norm_quantity_col) && col_map$norm_quantity_col %in% names(data_cln)) {
              data_cln <- data_cln |>
                dplyr::mutate(!!rlang::sym(col_map$norm_quantity_col) := as.numeric(!!rlang::sym(col_map$norm_quantity_col)))
            }
            if (!is.null(col_map$raw_quantity_col) && col_map$raw_quantity_col %in% names(data_cln)) {
              data_cln <- data_cln |>
                dplyr::mutate(!!rlang::sym(col_map$raw_quantity_col) := as.numeric(!!rlang::sym(col_map$raw_quantity_col)))
            }
            if (!is.null(col_map$quantity_col) && col_map$quantity_col %in% names(data_cln)) {
                data_cln <- data_cln |>
                dplyr::mutate(!!rlang::sym(col_map$quantity_col) := as.numeric(!!rlang::sym(col_map$quantity_col)))
            }
        ...
        shiny::observeEvent(input$assign_tech_reps, {
            ...
            if (length(input$tech_rep_samples) < 2) {
                shiny::showNotification("Please select at least two samples to assign as technical replicates.", type = "warning")
                return()
            }
            ...
            if (length(selected_groups) > 1 || any(is.na(selected_groups))) {
                shiny::showNotification("All selected samples must belong to the same group. Please assign metadata first.", type = "warning")
                return()
            }
            ...
            shiny::showNotification(paste("Assigned", length(input$tech_rep_samples), 
        ...
        shiny::observeEvent(input$reset_changes, {
            ...
            shiny::showModal(shiny::modalDialog(
                title = "Confirm Reset",
                shiny::HTML(paste0("<p>This will revert <strong>", input$reset_scope, "</strong> to their initial state. This action cannot be undone.</p>")),
        ...
        shiny::observeEvent(input$save_results, {
            ...
            shiny::showNotification("Design saved successfully. You can close this builder.", type="message", duration = 5)
        })
