...
mod_lipid_design_server <- function(id, workflow_data, experiment_paths, volumes = NULL, qc_trigger = NULL) {
    ...
    shiny::moduleServer(id, function(input, output, session) {
            ...
            tryCatch({
                ...
                if (file.exists(col_map_file)) {
                    ...
                    if (is.null(lipid_id_col)) {
                        lipid_id_col <- all_cols[1]  # Fallback to first column
                    }
                ...
                if (is.null(workflow_data$state_manager)) {
                    workflow_data$state_manager <- WorkflowState$new("lipidomics")
        ...
        if (exists("mod_lipid_design_builder_server")) {
            builder_results_rv <- mod_lipid_design_builder_server(
                "builder"
