...
enrichmentAnalysisAppletUI <- function(id) {
  ns <- NS(id)
...
#' @rdname enrichmentAnalysisAppletModule 
#' @export
enrichmentAnalysisAppletServer <- function(id, workflow_data, experiment_paths, omic_type, experiment_label, selected_tab = NULL) {
  shiny::moduleServer(id, function(input, output, session) {
    
    cat("--- Entering enrichmentAnalysisAppletServer ---\n")
    cat(sprintf("   enrichmentAnalysisAppletServer Arg: id = %s\n", id))
    cat(sprintf("   enrichmentAnalysisAppletServer Arg: workflow_data is NULL = %s\n", is.null(workflow_data)))
    cat(sprintf("   enrichmentAnalysisAppletServer Arg: selected_tab is NULL = %s\n", is.null(selected_tab)))
    
    cat("=== ENRICHMENT ANALYSIS MODULE SERVER STARTED ===\n")
    cat(sprintf("Module ID: %s\n", id))
    
    # Initialize reactive values for enrichment state
    enrichment_data <- reactiveValues(
    ...
    current_analysis_method <- reactive({
      req(input$organism_taxid)
    ...
    observe({
      ...
      req(enrichment_data$analysis_complete)
      req(enrichment_data$all_enrichment_results)
    ...
    if (!is.null(selected_tab)) {
      cat("   enrichmentAnalysisAppletServer Step: Setting up tab selection observer\n")
      ...
      cat("   enrichmentAnalysisAppletServer Step: No selected_tab parameter provided - tab selection observer NOT set up\n")
    }
    ...
    get_raw_contrast_name <- reactive({
      req(input$selected_contrast)
