...
deAnalysisWrapperFunction <- function(
  ...
  technical_replicate_id <- checkParamsObjectFunctionSimplify(theObject, "technical_replicate_id", "replicates")
  ...
  if (use_dpc_de && requireNamespace("limpa", quietly = TRUE)) {
    ...
    # Check if y_elist_filtered needs to be subsetted
    if (ncol(y_elist_filtered) != nrow(design_matrix_for_dpcde)) {
      cat("   DE ANALYSIS Step: Synchronizing EList dimensions with design matrix...\n")

      # Check coverage
      missing_samples <- setdiff(samples_in_design, colnames(y_elist_filtered))
      if (length(missing_samples) > 0) {
        stop(paste(
          "The following samples are in the design matrix but missing from DPC quantification data:",
          paste(missing_samples, collapse = ", ")
        ))
      }

      # Subset EList to match design matrix rows
      y_elist_filtered <- y_elist_filtered[, samples_in_design]
      cat("   DE ANALYSIS Step: EList subsetted to:", ncol(y_elist_filtered), "samples\n")
    }
  ...
  significant_rows <- getSignificantData(
    list_of_de_tables = list(contrasts_results_table),
