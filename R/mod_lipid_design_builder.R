...
mod_lipid_design_builder_server <- function(id, data_tbl, config_list, column_mapping
    ...
    shiny::moduleServer(id, function(input, output, session) {
        ...
        # == Helper: Extract Sample Names from Lipidomics Data List =============
        # Lipidomics data is a LIST of assays where samples are COLUMN NAMES
        ...
        get_sample_columns <- function(assay_list, col_map) {
            ...
            # Exclude lipid ID and annotation columns
            exclude_cols <- c(
                col_map$lipid_id_col
        ...
        initial_state <- shiny::reactive({
            ...
            if (length(sample_names) == 0) {
                logger::log_error("No sample columns detected in lipidomics data")
                return(NULL)
            }
        ...
        shiny::observeEvent(input$save_results, {
            ...
            data_cln_final <- lapply(current_data, function(assay_df) {
                # Keep lipid ID column and annotation column
                keep_cols <- c(
                    col_map$lipid_id_col
                    , col_map$annotation_col
                )
