...
generateLimpaQCPlots <- function(after_object,
  ...
  # Detect object type
  is_protein_object <- inherits(after_object, "ProteinQuantitativeData")
  is_peptide_object <- inherits(after_object, "PeptideQuantitativeData")

  # Check for protein-level limpa results FIRST if it's a protein object
  if (is_protein_object && !is.null(after_object@args$limpa_dpc_quant_results)) {
  ...
  # Also check for protein imputation results (Method 1) which was missing support
  else if (is_protein_object && !is.null(after_object@args$limpa_protein_imputation_results)) {
    dpc_results <- after_object@args$limpa_protein_imputation_results
    analysis_type <- "protein_imputation"
    if (verbose) cat("Detected protein-level limpa DPC imputation results\n")
  }

  # Then check for peptide-level limpa results (if dpc_results is still NULL)
  else if (!is.null(after_object@args$limpa_dpc_results)) {
  ...
  plot_list$missing_comparison <- tryCatch(
    {
      ...
      if (analysis_type == "peptide_imputation") {
        ...
        # Use design matrix to identify sample columns (Safest method)
        if (!is.null(after_object@design_matrix) && !is.null(after_object@sample_id)) {
          # Get sample IDs from design matrix (ensures only sample cols are picked)
          if (after_object@sample_id %in% colnames(after_object@design_matrix)) {
            sample_cols <- after_object@design_matrix[[after_object@sample_id]]
            # Only keep cols that exist in quant table
            sample_cols <- intersect(sample_cols, colnames(after_object@protein_quant_table))
          } else {
            # Fallback if sample_id not in design_matrix (invalid object state but safer to fallback)
            sample_cols <- setdiff(colnames(after_object@protein_quant_table), after_object@protein_id_column)
          }
        } else {
          # Fallback to excluding ID column
          protein_id_col <- after_object@protein_id_column
          sample_cols <- setdiff(colnames(after_object@protein_quant_table), protein_id_col)
        }

        if (length(sample_cols) == 0) {
          stop(paste("No sample columns found in protein_quant_table."))
        }

        after_matrix <- as.matrix(after_object@protein_quant_table[, sample_cols, drop = FALSE])
        # Ensure numeric matrix
        if (!is.numeric(after_matrix)) {
          if (verbose) cat("Warning: after_matrix is not numeric. Attempting conversion.\n")
          mode(after_matrix) <- "numeric"
        }
        ...
        if (!is.null(dpc_results$y_peptide_for_dpc)) {
          ...
          if (verbose) cat("Found stored y_peptide_for_dpc (dimensions:", paste(dim(before_matrix), collapse = "x"), ")\n")
      ...
      if (!is.null(before_matrix) && !is.null(after_matrix)) {
        ...
        if (analysis_type == "protein_quantification") {
          # Fix: If before_matrix has no colnames, assume they align if counts match
          if (is.null(colnames(before_matrix)) && ncol(before_matrix) == ncol(after_matrix)) {
            if (verbose) cat("Warning: before_matrix has no colnames. Assuming alignment with after_matrix based on column count.\n")
            colnames(before_matrix) <- colnames(after_matrix)
          }
          ...
          # Try fuzzy matching if standard intersection fails (e.g. Sample 1 vs Sample.1)
          if (length(common_samples) == 0) {
            norm_before <- make.names(colnames(before_matrix))
            norm_after <- make.names(colnames(after_matrix))
            common_norm <- intersect(norm_before, norm_after)

            if (length(common_norm) > 0) {
              if (verbose) cat("Warning: Sample names matched after normalization (make.names). Using normalized alignment.\n")
              # Align based on normalized names
              idx_before <- match(common_norm, norm_before)
              idx_after <- match(common_norm, norm_after)

              before_matrix <- before_matrix[, idx_before, drop = FALSE]
              after_matrix <- after_matrix[, idx_after, drop = FALSE]
              common_samples <- colnames(before_matrix) # These now effectively match
            }
          }

          # Force alignment if intersection is empty but counts match
          if (length(common_samples) == 0 && ncol(before_matrix) == ncol(after_matrix)) {
            if (verbose) cat("Warning: Sample names do not match but column counts are identical. Forcing alignment for QC plot.\n")
            colnames(before_matrix) <- colnames(after_matrix)
            common_samples <- colnames(after_matrix)
          }

          if (length(common_samples) == 0) {
            msg <- paste0("No common samples. Before: ", ncol(before_matrix), " cols, After: ", ncol(after_matrix), " cols.")
            # enhanced debugging on plot
            stop(msg)
          }
          ...
          if (length(common_samples) > 0) {
            ...
            # Enhanced debugging for empty intersection
            if (verbose) {
              cat("DEBUG: Sample Name Mismatch detected.\n")
              cat("Before Matrix Samples (first 5):", paste(head(colnames(before_matrix), 5), collapse = ", "), "\n")
              cat("After Matrix Samples (first 5):", paste(head(colnames(after_matrix), 5), collapse = ", "), "\n")

              # Show full list if small enough
              if (ncol(before_matrix) < 20 && ncol(after_matrix) < 20) {
                cat("Before(full): ", paste(colnames(before_matrix), collapse = ", "), "\n")
                cat("After(full): ", paste(colnames(after_matrix), collapse = ", "), "\n")
              }
            }
    ...
    error = function(e) {
      ...
      ggplot2::ggplot() +
        ggplot2::annotate("text", x = 0.5, y = 0.5, label = paste("Unavailable:\n", e$message), size = 3) +
        ggplot2::theme_void() +
        ggplot2::ggtitle("Missing Values Comparison")
    }
  )
  ...
  plot_list$intensity_distribution <- tryCatch(
    {
      ...
      if (!is.null(before_matrix) && !is.null(after_matrix)) {
        ...
        # Fix: If before_matrix has no colnames, assume they align if counts match (for protein quant)
        if (is.null(colnames(before_matrix)) && ncol(before_matrix) == ncol(after_matrix) && analysis_type == "protein_quantification") {
          if (verbose) cat("Warning (Intensity): before_matrix has no colnames. Assuming alignment with after_matrix.\n")
          colnames(before_matrix) <- colnames(after_matrix)
        }

        # Always check for common samples first to be safe
        common_samples <- intersect(colnames(before_matrix), colnames(after_matrix))
        ...
        if (length(common_samples) == 0) {
          ...
          if (length(common_norm) > 0) {
            if (verbose) cat("Warning (Intensity): Sample names matched after normalization. Using normalized alignment.\n")
            idx_before <- match(common_norm, norm_before)
            idx_after <- match(common_norm, norm_after)

            before_matrix <- before_matrix[, idx_before, drop = FALSE]
            after_matrix <- after_matrix[, idx_after, drop = FALSE]
            common_samples <- colnames(before_matrix)
          }
        }

        # Force alignment if intersection is empty but counts match (AGRESSIVE FIX for Intensity)
        if (length(common_samples) == 0 && ncol(before_matrix) == ncol(after_matrix)) {
          if (verbose) cat("Warning (Intensity): Sample names do not match but column counts are identical. Forcing alignment.\n")
          colnames(before_matrix) <- colnames(after_matrix)
          common_samples <- colnames(after_matrix)
        }

        if (length(common_samples) == 0) {
          # Enhanced debug info for plot
          msg <- paste0("No common samples. Before: ", ncol(before_matrix), " cols, After: ", ncol(after_matrix), " cols.")
          stop(msg)
        }
        ...
        if (length(common_samples) > 0) {
          ...
          before_matrix <- before_matrix[, common_samples, drop = FALSE]
          after_matrix <- after_matrix[, common_samples, drop = FALSE]
        } else if (verbose) {
          # Debugging fallback already handled by stop() above
        }

        if (!identical(dim(before_matrix), dim(after_matrix))) {
          common_features <- intersect(rownames(before_matrix), rownames(after_matrix))
          if (length(common_features) > 0) {
            before_matrix <- before_matrix[common_features, , drop = FALSE]
            after_matrix <- after_matrix[common_features, , drop = FALSE]
          }
        }
    ...
    tryCatch(
      {
        ...
        MultiScholaR::savePlot(composite_plot, save_dir, paste0(plot_prefix, "_composite"), width = 10, height = 10)
