...
metabWorkflowAssayNames <- function(workflow_data, name = "data_tbl") {
...
settlePersistedMetabArtifactWorkflowSafely <- function(
    workflow_data,
    log_warn = logger::log_warn
) {
    ...
    tryCatch({
        ...
        applyMetabResumeBundle(workflow_data, bundle)
        settleMetabArtifactWorkflowSafely(workflow_data, log_warn = log_warn)
    }, error = \(error) {
        log_warn(paste(
            "metabolomics post-design settlement failed; memory state retained:",
            conditionMessage(error)
        ))
        list(
            enabled = TRUE,
