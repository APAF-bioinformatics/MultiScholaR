...
mod_lipid_norm_impute_server <- function(id, workflow_data, omic_type, experiment_label) {
    shiny::moduleServer(id, function(input, output, session) {
        ...
        shiny::observeEvent(input$apply_imputation, {
            ...
            if (input$impute_method == "none") return()
        ...
        output$limpa_diag_text <- shiny::renderPrint({ diag_data$dpc_plots })
        output$missforest_diag_text <- shiny::renderPrint({ diag_data$missforest_logs })
    })
}
