...
#' @title Lipid-Level Missing Value Imputation using limpa Package
#' @name lipidMissingValueImputationLimpa,LipidomicsAssayData-method
#' @importFrom purrr map map_dfc
#' @importFrom tibble column_to_rownames rownames_to_column
#' @importFrom logger log_info log_warn log_error
#' @export
setMethod(
    f = "lipidMissingValueImputationLimpa",
    ...
    definition = function(theObject,
        ...
        if (!requireNamespace("limpa", quietly = TRUE)) {
            stop("limpa package is required but not installed.")
        }
        ...
        if (verbose) {
            logger::log_info("Starting limpa-based lipid-level missing value imputation...")
        }
        ...
        theObject@lipid_data <- purrr::map(names(assay_list), function(assay_name) {
            ...
            assay_matrix <- assay_df |>
                ...
                tibble::column_to_rownames(lipid_id_col) |>
                as.matrix()
            ...
            y_elist <- list(
                E = assay_matrix,
                genes = data.frame(
                    lipid.id = rownames(assay_matrix),
            ...
            quant_result <- if (!is.null(dpc_params)) {
                limpa::dpcQuant(y = y_elist, protein.id = "lipid.id", dpc = dpc_params, verbose = verbose, chunk = chunk)
            } else {
                limpa::dpcQuant(y = y_elist, protein.id = "lipid.id", dpc.slope = dpc_slope, verbose = verbose, chunk = chunk)
            }

            imputed_matrix <- quant_result$E
            rownames(imputed_matrix) <- quant_result$genes$lipid.id
            ...
            metadata_cols <- setdiff(colnames(assay_df), sample_cols)
            ...
            final_df <- dplyr::left_join(assay_df_metadata, imputed_df, by = lipid_id_col)
            return(final_df)
        }) |> purrr::set_names(names(assay_list))

        if (verbose) logger::log_info("Lipid imputation complete.")
        return(theObject)
    }
)

#' @title Lipid-Level Missing Value Imputation using missForest Package
#' @name lipidMissingValueImputationMissForest,LipidomicsAssayData-method
#' @importFrom purrr map
#' @importFrom tibble column_to_rownames rownames_to_column
#' @importFrom logger log_info log_warn log_error
#' @export
setMethod(
    f = "lipidMissingValueImputationMissForest",
    signature = "LipidomicsAssayData",
    definition = function(theObject,
                          maxiter = 10,
                          ntree = 100,
        ...
        if (!requireNamespace("missForest", quietly = TRUE)) {
            stop("missForest package is required but not installed.")
        }
        ...
        if (verbose) {
            logger::log_info("Starting missForest-based lipid-level missing value imputation...")
        }
        ...
        if (verbose) logger::log_info("Lipid missForest imputation complete.")
        return(theObject)
    }
)
...
#' @name plotDensity,LipidomicsAssayData-method
#' @importFrom purrr map set_names
...
setMethod(
    ...
    definition = function(theObject,
        ...
        ## Compare the different experimental groups and obtain lists of differentially expressed lipids for EACH assay
        assay_results_list <- purrr::map(names(theObject@lipid_data), function(assay_name) {
            message(sprintf("   differentialAbundanceAnalysisHelper: Processing assay: %s", assay_name))
            
            # Prepare data matrix for DE analysis
            matrix_data <- as.matrix(theObject@lipid_data[[assay_name]][, -1]) # Exclude ID column
            colnames(matrix_data) <- colnames(theObject@lipid_data[[assay_name]])[-1]
            rownames(matrix_data) <- theObject@lipid_data[[assay_name]][[theObject@lipid_id_column]]
        ...
        names(assay_results_list) <- names(theObject@lipid_data)
        
        # Format results for LipidomicsDifferentialAbundanceResults S4 object
        formatted_results_list <- purrr::map(assay_results_list, function(assay_res) {
            assay_res$results |>
                purrr::map(\(result_table) {
                    result_table |>
                        tibble::rownames_to_column(var = theObject@lipid_id_column)
                })
        })
...
setMethod(
    ...
    definition = function(
      ...
      lipids_intensity_cutoff_percentile = NULL, ...
        ...
        min_samples_per_group <- checkParamsObjectFunctionSimplify(theObject, "min_samples_per_group", 3)
        min_groups <- checkParamsObjectFunctionSimplify(theObject, "min_groups", 2)
        lipids_intensity_cutoff_percentile <- checkParamsObjectFunctionSimplify(theObject, "lipids_intensity_cutoff_percentile", 1)

        # Update Object Parameters
        theObject <- updateParamInObject(theObject, "grouping_variable")
        theObject <- updateParamInObject(theObject, "min_samples_per_group")
        theObject <- updateParamInObject(theObject, "min_groups")
        theObject <- updateParamInObject(theObject, "lipids_intensity_cutoff_percentile")
        ...
        if (nrow(small_groups) > 0) {
            msg <- sprintf(
                "Warning: The following groups have fewer samples than 'min_samples_per_group' (%d): %s. Features in these groups can never pass the threshold.",
                min_samples_per_group,
                paste(small_groups[[grouping_variable]], collapse = ", ")
            )
            logger::log_warn(msg)
            warning(msg)
        }
        ...
        theObject@lipid_data <- purrr::map(names(original_assay_list), function(assay_name) {
            ...
            assay_long <- assay_table |>
                dplyr::select(all_of(c(lipid_id_col, sample_cols))) |>
                tidyr::pivot_longer(cols = all_of(sample_cols), names_to = sample_id_col, values_to = "Intensity") |>
                dplyr::left_join(design_matrix |> dplyr::select(all_of(c(sample_id_col, grouping_variable))), by = sample_id_col)

            # Count samples above threshold per feature per group
            passing_features <- assay_long |>
                dplyr::group_by(!!rlang::sym(lipid_id_col), !!rlang::sym(grouping_variable)) |>
                dplyr::summarise(num_passing = sum(!is.na(Intensity) & Intensity >= threshold, na.rm = TRUE), .groups = "drop") |>
                dplyr::group_by(!!rlang::sym(lipid_id_col)) |>
                dplyr::summarise(num_groups_passing = sum(num_passing >= min_samples_per_group), .groups = "drop") |>
                dplyr::filter(num_groups_passing >= min_groups) |>
                dplyr::pull(!!rlang::sym(lipid_id_col))

            filtered_assay <- assay_table |>
                dplyr::filter(!!rlang::sym(lipid_id_col) %in% passing_features)

            message(sprintf("   Features: %d -> %d", nrow(assay_table), nrow(filtered_assay)))
            return(filtered_assay)
        }) |> purrr::set_names(names(original_assay_list))

        message("--- Exiting lipidIntensityFiltering ---")
        return(theObject)
    }
)
...
setMethod(
    ...
    definition = function(theObject, pearson_correlation_per_pair = NULL, min_pearson_correlation_threshold = 0.5) {
        ...
        # Instead of super-assignment inside map, collect results and then unlist
        removal_list <- purrr::map2(assay_list, pearson_correlation_per_pair, function(current_assay_data, correlation_results) {
            if (is.null(correlation_results) || nrow(correlation_results) == 0) {
                ...
                return(list(data = current_assay_data, removed = character()))
            }
            ...
            return(list(data = current_assay_data, removed = samples_to_remove))
        })

        # Extract results
        theObject@lipid_data <- purrr::map(removal_list, "data") |> purrr::set_names(names(assay_list))
        samples_to_remove_total <- unique(unlist(purrr::map(removal_list, "removed")))

        if (length(samples_to_remove_total) > 0) {
            theObject@design_matrix <- design_matrix |>
                dplyr::filter(!(!!rlang::sym(sample_id_col_name) %in% samples_to_remove_total))
            message(sprintf("Total samples removed across all assays: %d", length(samples_to_remove_total)))
        }
