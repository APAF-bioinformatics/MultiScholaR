...
setMethod(f = "plotRle",
          ...
          definition = function(theObject, grouping_variable, yaxis_limit = c(), sample_label = NULL) {
            ...
            # Convert metabolite_data to matrix, ensuring it's numeric
            message("   plotRle: Converting metabolite_data to matrix...")
            assay_data <- as.matrix(theObject@metabolite_data[, -1])  # Exclude the Name column
            if (!is.numeric(assay_data)) {
              stop("Metabolite data must be numeric")
            }

            # Get sample IDs and grouping information
            design_matrix_df <- as.data.frame(theObject@design_matrix)
            sample_id_col_name <- theObject@sample_id

            # Ensure sample columns match design matrix
            sample_cols <- intersect(colnames(assay_data), design_matrix_df[[sample_id_col_name]])
            if (length(sample_cols) == 0) {
              stop("No matching sample columns found between assay data and design matrix")
            }

            # Filter and order data
            assay_data <- assay_data[, sample_cols, drop = FALSE]
            design_matrix_filtered <- design_matrix_df[design_matrix_df[[sample_id_col_name]] %in% sample_cols, ]

            # Get grouping information
            rowinfo_vector <- design_matrix_filtered[[grouping_variable]]

            # Call plotRleHelper with numeric matrix
            message("   plotRle: Calling plotRleHelper...")
            plotRleHelper(
              Y = t(assay_data),  # Transpose to get samples in rows
...
#             message("   plotRle: theObject structure:")
#             str(theObject)
#
#             # --- Plotting Logic per Assay ---
