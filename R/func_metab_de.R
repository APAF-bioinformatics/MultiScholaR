...
runMetabolitesDE <- function(
    ...
    for (assay_name in assay_names) {
        ...
        logger::log_info(sprintf("   Expression matrix: %d metabolites x %d samples"
...
createMetabDeResultsLongFormat <- function(
    ...
    logger::log_info(sprintf("   expr_matrix: %d metabolites x %d samples"
        , nrow(expr_matrix), ncol(expr_matrix)))
    ...
    logger::log_info(sprintf("   intensity_wide: %d metabolites x %d columns"
...
generateMetabDEHeatmap <- function(
    ...
    expr_list <- lapply(assays_to_use, function(assay_name) {
        ...
        if (length(assay_metab_ids) == 0) return(NULL)
    ...
    for (assay_name in assays) {
        ...
        for (contrast_name in contrasts) {
            ...
            tryCatch({
                ...
                priority_cols <- c("metabolite_id", "metabolite_name"
            ...
            tryCatch({
                ...
                if (!is.null(heatmap_obj)) {
                    ...
                    logger::log_warn(sprintf("      No significant metabolites for heatmap: %s / %s"
