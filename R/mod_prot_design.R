...
mod_prot_design_server <- function(id, workflow_data, experiment_paths, volumes = NULL, qc_trigger = NULL) {
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    import_fasta_path <- shiny::reactiveVal(NULL)
    ...
    shiny::observeEvent(input$import_fasta_file, {
      if (!is.null(input$import_fasta_file) && !is.integer(input$import_fasta_file)) {
        ...
        if (nrow(file_info) > 0) {
          ...
          output$import_fasta_file_path <- shiny::renderText(selected_path)
        }
      }
    })
        ...
        if (!file.exists(config_path)) {
          ...
          tryCatch({
            ...
            if (file.exists(pkg_config) && pkg_config != "") {
              logger::log_info(paste("Found default config.ini in package:", pkg_config))
              file.copy(pkg_config, config_path)
              logger::log_info(paste("Default config.ini copied to:", config_path))
              shiny::showNotification("Using default config.ini (from package).", type = "message")
            } else {
              ...
              shiny::showNotification("Using default config.ini (downloaded).", type = "message")
            }
    ...
    # Using mod_prot_design_builder_server
    if (exists("mod_prot_design_builder_server")) {
      builder_results_rv <- mod_prot_design_builder_server(
        "builder",
    ...
    shiny::observeEvent(builder_results_rv(), {
      ...
      tryCatch({
          ...
          assay_manifest_path <- file.path(source_dir, "assay_manifest.txt")
          writeLines("Proteomics", assay_manifest_path)
          logger::log_info(paste("Saved assay_manifest.txt to:", assay_manifest_path))
          
          col_map_path <- file.path(source_dir, "column_mapping.json")
          col_map_data <- workflow_data$column_mapping
          if (is.null(col_map_data)) {
              col_map_data <- list(assay_type = "proteomics")
          }
          jsonlite::write_json(col_map_data, col_map_path, auto_unbox = TRUE, pretty = TRUE)
          logger::log_info(paste("Saved column_mapping.json to:", col_map_path))
