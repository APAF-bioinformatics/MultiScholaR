  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$fasta_detection_status <- shiny::renderText({
      ...
      if (length(import_path) > 0 && dir.exists(import_path)) {
        fasta_files <- list.files(import_path,
      ...
      tryCatch(
        {
          ...
          if (workflow_type == "DIA") {
            ...
            required_cols <- c(
              workflow_data$column_mapping$protein_col,
            ...
            logger::log_info(sprintf(
              "%s Import: Pivoted data from %d rows (long) to %d rows (wide)",
              workflow_type,
              nrow(workflow_data$data_cln),
              nrow(protein_quant_table_wide)
            ))
            logger::log_info(sprintf(
              "%s Import: Wide format has %d protein rows, %d sample columns",
              workflow_type,
              nrow(protein_quant_table_wide),
              ncol(protein_quant_table_wide) - 1
            )) # -1 for protein ID column
