  ...
  if (getOption("multischolar.capture_test_checkpoints", FALSE)) {
    tryCatch({
      base_dir <- getOption("multischolar.checkpoint_dir",
                            file.path("tests", "testdata", "sepsis", "proteomics"))
...
# Fixtures: tests/testdata/sepsis/proteomics/cp01_raw_imported.rds
...
test_that("importDIANNData returns expected structure", {
  ...
  fixture_path <- testthat::test_path("..", "testdata", "sepsis", "proteomics",
...
test_that("QC filtering reduces data appropriately (CP01 -> CP02)", {
  cp01_path <- testthat::test_path("..", "testdata", "sepsis", "proteomics",
                                   "cp01_raw_imported.rds")
  cp02_path <- testthat::test_path("..", "testdata", "sepsis", "proteomics",
...
1. Create `tests/testdata/<dataset>/<omics_layer>/` directory (e.g., `tests/testdata/sepsis/proteomics/`)
...
1. **Checkpoint capture**: Launch the app, enable capture, run a workflow, verify `.rds` files appear in the new structured directory (e.g., `tests/testdata/sepsis/proteomics/`)
