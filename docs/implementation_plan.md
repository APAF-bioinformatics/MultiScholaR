...
I have identified several function groups that can be merged into the base class hierarchy to reduce duplication:

### 1. QC Plotting (`plotPca`, `plotRle`, `plotDensity`, `plotPearson`)
*   **Base Class (`OmicData`)**: Implement input validation and design matrix checks.
*   **`SingleAssayOmicData`**: Implement the actual plotting logic for a single data table.
*   **`MultiAssayOmicData`**: Implement the logic to iterate over `assay_list` and apply the same plotting logic to each assay.

### 2. Intensity Filtering (`*IntensityFiltering`)
*   Unify `proteinIntensityFiltering`, `peptideIntensityFiltering`, `lipidIntensityFiltering`, and `metaboliteIntensityFiltering` into a single `intensityFiltering` generic.
*   Move shared parameter resolution logic (`checkParamsObjectFunctionSimplify`) to the base class.

### 3. Design Matrix Cleaning (`cleanDesignMatrix*`)
*   Unify `cleanDesignMatrix` and `cleanDesignMatrixPeptide` into a single `cleanDesignMatrix` method in the `OmicData` base class.

### 4. Normalization
*   Unify `normaliseBetweenSamples` and `normaliseUntransformedData` under a single `normaliseData` generic, allowing for omic-specific method dispatch (e.g., `cyclicloess` for proteins, `ITSD` for metabolites).
