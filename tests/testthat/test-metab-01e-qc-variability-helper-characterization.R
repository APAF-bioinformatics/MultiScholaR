...
loadSelectedFunctions(
  paths = c(
    file.path(repo_root, "R", "func_metab_qc_variability_helpers.R"),
...
test_that("metabolomics QC variability helpers preserve grouped CV and IS metric contracts", {
  ...
  expect_equal(cv_metrics$cv[c(1, 3)], c(47.1404520791032, 47.1404520791032), tolerance = 1e-9)
  ...
  expect_equal(is_metrics$cv, c(28.5714285714286, 9.09090909090909), tolerance = 1e-9)
})

test_that("metabolomics QC variability helpers keep current empty-result fallbacks", {
  cv_assay_data <- data.frame(
    metabolite_id = c("M1", "M2"),
    Sample1 = c(10, 5),
    Sample2 = c(20, 10),
