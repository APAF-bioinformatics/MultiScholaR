module_ci_lipid_da_samples <- function() {
  paste0("S", seq_len(6))
}

module_ci_lipid_da_design <- function(samples = module_ci_lipid_da_samples()) {
  data.frame(
    Run = samples,
...
module_ci_lipid_da_feature_ids <- function() {
  c("L_UP", "L_DOWN", "L_NS", "L_CONST", "L_TIED", "L_METALESS")
}

module_ci_lipid_da_assay <- function(samples = module_ci_lipid_da_samples(),
                                     assay_shift = 0,
                                     missing_metadata = FALSE) {
  values <- rbind(
    L_UP = c(10, 11, 31, 32, 18, 19),
    L_DOWN = c(50, 52, 22, 23, 43, 44),
    L_NS = c(20, 21, 22, 21, 20, 22),
    L_CONST = c(7, 7, 7, 7, 7, 7),
    L_TIED = c(13, 14, 28, 29, 23, 24),
    L_METALESS = c(16, 17, 12, 13, 19, 20)
  )
  ...
  assay <- data.frame(
    ...
    LipidClass = rep(c("PC", "TG", "SM"), length.out = nrow(values)),
    stringsAsFactors = FALSE
  )
  if (!isTRUE(missing_metadata)) {
    assay$lipid <- paste("Lipid", rownames(values))
  }
  cbind(assay, sample_df)
}

module_ci_lipid_da_assays <- function(layout = c("pos_neg", "pos_gcms", "single_pos", "single_gcms", "triple"),
                                      missing_metadata = FALSE) {
  layout <- match.arg(layout)
  pos <- module_ci_lipid_da_assay(assay_shift = 0, missing_metadata = missing_metadata)
  neg <- module_ci_lipid_da_assay(assay_shift = 3, missing_metadata = missing_metadata)
  gc <- module_ci_lipid_da_assay(assay_shift = 5, missing_metadata = missing_metadata)

  switch(
    layout,
    single_pos = list(LCMS_Pos = pos),
    single_gcms = list(GCMS = gc),
    pos_neg = list(LCMS_Pos = pos, LCMS_Neg = neg),
    pos_gcms = list(LCMS_Pos = pos, GCMS = gc),
    triple = list(LCMS_Pos = pos, LCMS_Neg = neg, GCMS = gc)
  )
}

module_ci_lipid_da_object <- function(layout = c("pos_gcms", "pos_neg", "single_pos", "single_gcms", "triple"),
                                      missing_metadata = FALSE,
                                      formula_string = "~ 0 + group") {
  layout <- match.arg(layout)
  methods::new(
    "LipidomicsAssayData",
    lipid_data = module_ci_lipid_da_assays(
      layout = layout,
      missing_metadata = missing_metadata
    ),
    lipid_id_column = "lipid_id",
    annotation_id_column = if (isTRUE(missing_metadata)) "missing_lipid" else "lipid",
    database_identifier_type = "lipid_id",
    internal_standard_regex = "^IS_",
    design_matrix = module_ci_lipid_da_design(),
    sample_id = "Run",
    group_id = "group",
    technical_replicate_id = "tech_rep_group",
    args = list(
      daAnalysisParameters = list(formula_string = formula_string),
      module_ci_fixture = "MCI-022"
    )
  )
}

module_ci_lipid_da_contrasts <- function(case = c(
...
module_ci_lipid_da_state_manager <- function(current_state,
                                             current_state_name = "lipid_correlation_filtered") {
  states <- list()
  saved <- list()
...
module_ci_lipid_da_workflow <- function(current_object = module_ci_lipid_da_object(),
                                        contrasts = module_ci_lipid_da_contrasts("multi_group")) {
  list2env(list(
    state_manager = module_ci_lipid_da_state_manager(current_object),
...
module_ci_lipid_da_data_env <- function() {
  list2env(list(
...
module_ci_lipid_da_session_data <- function(layout = c("pos_gcms", "pos_neg", "single_pos", "single_gcms", "triple"),
                                            assay_names = NULL,
                                            missing_metadata = FALSE,
                                            contrasts = module_ci_lipid_da_contrasts("multi_group")) {
  layout <- match.arg(layout)
  object <- module_ci_lipid_da_object(layout = layout, missing_metadata = missing_metadata)
  list(
    omic_type = "lipidomics",
    current_s4_object = object,
    r6_current_state_name = "lipid_correlation_filtered",
    contrasts_tbl = contrasts,
    assay_names = if (is.null(assay_names)) names(object@lipid_data) else assay_names,
    feature_counts = vapply(object@lipid_data, nrow, integer(1)),
    normalization_complete = TRUE,
    correlation_filtering_complete = TRUE
  )
}

module_ci_lipid_da_capture_ui <- function() {
  capture <- new.env(parent = emptyenv())
  capture$select_updates <- list()
  capture$text_updates <- list()
  capture$notifications <- list()
  capture$removed <- character()
  capture$updateSelectInput <- function(session, inputId, choices = NULL, selected = NULL, ...) {
  ...
  capture$updateTextAreaInput <- function(session, inputId, value = NULL, ...) {
...
module_ci_lipid_da_restore_post_read <- function(capture) {
  function(sessionData, workflowData, daData, session, debugLog) {
    restoreLipidDaPostReadSessionState(
      ...
      restoreFormulaFn = function(sessionData, daData, session, debugLog, ...) {
        restoreLipidDaFormulaFromSession(
          ...
          daData = daData,
          session = session,
          updateTextAreaInputFn = capture$updateTextAreaInput,
          debugLog = debugLog,
...
module_ci_lipid_da_local_binding <- function(env, name, value, .local_envir = parent.frame()) {
...
module_ci_lipid_da_stats_table <- function(feature_ids,
                                           case = c(
...
module_ci_lipid_da_bind_orchestration_stubs <- function(case = "mixed",
    ...
    function(data,
      ...
      stats <- stats::setNames(
        lapply(contrast_strings, function(...) {
          module_ci_lipid_da_stats_table(rownames(data), case = case)
        }),
        contrast_strings
      )
      list(
        results = stats,
        fit.eb = structure(list(case = case), class = "module_ci_lipid_da_fit"),
...
module_ci_lipid_da_results_long <- function(case = c(
  ...
  rows <- list()
  for (assay_name in names(object@lipid_data)) {
    assay <- object@lipid_data[[assay_name]]
    feature_ids <- as.character(assay$lipid_id)
    for (contrast_idx in seq_len(nrow(contrasts))) {
      stats <- module_ci_lipid_da_stats_table(feature_ids, case = case)
      stats$lipid_id <- rownames(stats)
      stats$lipid_name <- paste("Lipid", stats$lipid_id)
...
module_ci_lipid_da_results_list <- function(case = "mixed") {
  long <- module_ci_lipid_da_results_long(case = case)
  list(
    theObject = module_ci_lipid_da_object(layout = "triple"),
...
module_ci_lipid_da_assert_long_schema <- function(results) {
