...
test_that("module CI manifest filters scenarios by omic, module, runtime, and ticket", {
  ...
  lipid_summary <- module_ci_scenarios(
    manifest,
    omic = "lipidomics",
    module_family = "summary_report",
    runtime = "unit-contract"
  ...
  expect_identical(
    module_ci_scenario_ids(lipid_summary),
    "MCI-023.1-lipidomics-summary-report-schema-smoke"
  )
  expect_true("module-ci-lipid-summary" %in% lipid_summary[[1]]$required_tests)

  cross_module <- module_ci_scenarios(
    manifest,
    omic = "all",
    module_family = "cross_module_integrity",
    runtime = "unit-contract"
  )
  expect_identical(
    module_ci_scenario_ids(cross_module),
