...
getLipidQcServer <- function() {
  package_ns <- asNamespace("MultiScholaR")
  if (exists("mod_lipid_qc_server", envir = package_ns, inherits = FALSE)) {
    return(get("mod_lipid_qc_server", envir = package_ns, inherits = FALSE))
  }

  mod_lipid_qc_server
}

newSharedLipidQcCapture <- function() {
...
makeSharedLipidQcWorkflow <- function(state = NULL, state_error = NULL) {
  state_manager <- list(
    getState = function() {
...
localSharedLipidQcNamespaceBinding <- function(env, name, value, .local_envir = parent.frame()) {
...
withSharedLipidQcSubmoduleMocks <- function(server_env, captured) {
  ...
  localSharedLipidQcNamespaceBinding(
    server_env, "mod_lipid_qc_intensity_server",
    record_server("intensity"), mock_frame
  )
  localSharedLipidQcNamespaceBinding(
    server_env, "mod_lipid_qc_duplicates_server",
    record_server("duplicates"), mock_frame
  )
  localSharedLipidQcNamespaceBinding(
    server_env, "mod_lipid_qc_itsd_server",
    record_server("itsd"), mock_frame
  )
  localSharedLipidQcNamespaceBinding(
    server_env, "mod_lipid_qc_s4_server",
    record_server("s4_finalize"), mock_frame
  )
  localSharedLipidQcNamespaceBinding(
    server_env, "mod_lipid_qc_intensity_ui",
    record_ui("intensity"), mock_frame
  )
  localSharedLipidQcNamespaceBinding(
    server_env, "mod_lipid_qc_duplicates_ui",
    record_ui("duplicates"), mock_frame
  )
  localSharedLipidQcNamespaceBinding(
    server_env, "mod_lipid_qc_itsd_ui",
    record_ui("itsd"), mock_frame
  )
  localSharedLipidQcNamespaceBinding(
    server_env, "mod_lipid_qc_s4_ui",
    record_ui("s4_finalize"), mock_frame
  )
}

withSharedLipidQcShinyMocks <- function(captured,
...
test_that("lipidomics QC orchestrator preserves the unavailable-state info panel", {
  captured <- newSharedLipidQcCapture()
  server_fn <- getLipidQcServer()
  server_env <- environment(server_fn)

  withSharedLipidQcSubmoduleMocks(server_env, captured)
  withSharedLipidQcShinyMocks(captured, evaluate_observe = TRUE)

  workflow_data <- makeSharedLipidQcWorkflow(state_error = "state unavailable")
  server_fn(
    id = "qc",
...
test_that("lipidomics QC orchestrator preserves populated tab rendering", {
  captured <- newSharedLipidQcCapture()
  server_fn <- getLipidQcServer()
  server_env <- environment(server_fn)

  withSharedLipidQcSubmoduleMocks(server_env, captured)
  withSharedLipidQcShinyMocks(captured, evaluate_observe = FALSE)

  workflow_data <- makeSharedLipidQcWorkflow(state = list(status = "ready"))
  server_fn(
    id = "qc",
    workflow_data = workflow_data,
    experiment_paths = list(),
  ...
  expect_identical(captured$output$dynamic_qc_tabs$id, "qc-lipid_qc_tabs")
  expect_identical(
    captured$ui_ids,
    c(
...
test_that("lipidomics QC orchestrator preserves qc-trigger submodule initialization", {
  captured <- newSharedLipidQcCapture()
  server_fn <- getLipidQcServer()
  server_env <- environment(server_fn)
  ...
  workflow_data <- makeSharedLipidQcWorkflow(state = NULL)
  server_fn(
    id = "qc",
    workflow_data = workflow_data,
    experiment_paths = list(),
  ...
  expect_true(all(vapply(captured$server_calls, `[[`, character(1), "omicType") == "lipidomics"))
  expect_true(all(vapply(captured$server_calls, `[[`, character(1), "experimentLabel") == "Lipid Experiment"))
  expect_true("Lipidomics QC: Initializing sub-module servers" %in% captured$log_info)
  expect_true("Lipidomics QC: Sub-modules initialized successfully" %in% captured$log_info)
})

test_that("lipidomics QC orchestrator preserves state-detected auto-initialization", {
  captured <- newSharedLipidQcCapture()
  server_fn <- getLipidQcServer()
  server_env <- environment(server_fn)
  ...
  workflow_data <- makeSharedLipidQcWorkflow(
    state = structure(list(status = "ready"), class = "LipidomicsAssayData")
  )
  server_fn(
    id = "qc",
    workflow_data = workflow_data,
  ...
  expect_true("Lipidomics QC: Auto-initializing sub-modules (state detected)" %in% captured$log_info)
})
