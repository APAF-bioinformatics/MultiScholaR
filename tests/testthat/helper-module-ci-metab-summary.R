module_ci_metab_summary_config <- function(layout = c("combined", "lc", "gc"),
  ...
  list(
    layout = layout,
    workflow_type = "metabolomics",
    report_template = "metabolomics_report.rmd",
    normalization_method = normalization_method,
    ruv_mode = ruv_mode,
    itsd_method = itsd_method,
    da_q_cutoff = da_q_cutoff,
    da_logfc_cutoff = 1,
    formula_string = if (identical(layout, "gc")) "~ 0 + group + batch" else "~ 0 + group"
  )
}

module_ci_metab_summary_args <- function(config = module_ci_metab_summary_config()) {
  list(
    ...
    itsdNormalizationParameters = list(
      method = config$itsd_method,
      aggregation = "sum",
      regex = "^IS_"
    ),
    ruv = if (identical(config$ruv_mode, "skip")) {
    ...
    daAnalysisParameters = list(
      ...
      contrasts = module_ci_metab_da_contrasts("multi_group")$contrasts
    )
  )
}

module_ci_metab_summary_object <- function(config = module_ci_metab_summary_config()) {
  object <- module_ci_metab_da_object(
    layout = config$layout,
    formula_string = config$formula_string
  )
  object@args <- module_ci_metab_summary_args(config)
  object
}

module_ci_metab_summary_state_manager <- function(object,
                                                  state = "metab_correlation_filtered",
  ...
  manager$state_history <- c("metab_qc_complete", "metab_normalized", state)
...
module_ci_metab_summary_workflow <- function(config = module_ci_metab_summary_config()) {
  object <- module_ci_metab_summary_object(config)
  workflow <- new.env(parent = emptyenv())
  workflow$state_manager <- module_ci_metab_summary_state_manager(
    object,
    state = if (identical(config$ruv_mode, "skip")) "metab_normalized" else "metab_correlation_filtered",
    state_config = list(ruv_optimization_result = module_ci_metab_summary_args(config)$ruv)
  )
  workflow$config_list <- list(globalParameters = module_ci_metab_summary_args(config)$globalParameters)
  ...
  workflow$itsd_ui_params <- module_ci_metab_summary_args(config)$itsdNormalizationParameters
  workflow$ruv_optimization_result <- module_ci_metab_summary_args(config)$ruv
  workflow
}

module_ci_metab_summary_paths <- function() {
  root <- tempfile("module-ci-metab-summary-")
  paths <- list(
    base_dir = root,
...
module_ci_metab_summary_write_template <- function(project_dirs) {
  template_dir <- file.path(project_dirs$metabolomics$base_dir, "scripts", "metabolomics")
  dir.create(template_dir, recursive = TRUE, showWarnings = FALSE)
  writeLines(c("---", "title: module ci metabolomics report", "---"), file.path(template_dir, "metabolomics_report.rmd"))
  invisible(template_dir)
}

module_ci_metab_summary_write_publication_inputs <- function(project_dirs,
                                                             config = module_ci_metab_summary_config(),
                                                             include_optional = TRUE,
                                                             ruv_applied = !identical(config$ruv_mode, "skip")) {
  paths <- project_dirs$metabolomics
  object <- module_ci_metab_summary_object(config)
  da_long <- module_ci_metab_da_results_long("mixed")
  normalised_results <- data.frame(
    database_identifier = module_ci_metab_da_feature_ids()[seq_len(3)],
    assay = rep(names(object@metabolite_data), length.out = 3),
    Run = module_ci_metab_da_samples()[seq_len(3)],
    normalised_intensity = c(10.5, 20.5, 30.5),
    stringsAsFactors = FALSE
  )

  for (assay_name in unique(da_long$assay)) {
    assay_mode <- if (identical(assay_name, "GCMS")) "gcms" else "posmode"
    assay_rows <- da_long[da_long$assay == assay_name & da_long$comparison == "groupKO-groupWT", , drop = FALSE]
    file_base <- sprintf("de_%s_metabolites_groupKO-groupWT_long_annot", assay_mode)
  ...
  enrichment <- data.frame(
    pathway = c("glycolysis", "tca_cycle"),
    p_value = c(0.001, 0.02),
    q_value = c(0.01, 0.04),
    stringsAsFactors = FALSE
  )
  readr::write_tsv(enrichment, file.path(paths$pathway_dir, "KO_vs_WT_up_enrichment_results.tsv"))
  readr::write_tsv(enrichment, file.path(paths$pathway_dir, "KO_vs_WT_down_enrichment_results.tsv"))
  ...
  readr::write_tsv(module_ci_metab_da_contrasts("multi_group"), file.path(paths$source_dir, "contrasts_tbl.tab"))
  ...
  if (isTRUE(include_optional)) {
    ...
    writeBin(as.raw(c(0x89, 0x50, 0x4e, 0x47)), file.path(paths$feature_qc_dir, "gcms_post_norm_density.png"))
    writeBin(as.raw(c(0x89, 0x50, 0x4e, 0x47)), file.path(paths$feature_qc_dir, "lcms_pos_ruv_corrected_rle.png"))
    ...
    for (subdir in c("Interactive_Volcano_Plots", "NumSigDaMolecules", "Volcano_Plots", "Heatmap", "Heatmaps")) {
    ...
    writeLines(
      c("comparison\tsignificant", "KO_vs_WT\t2"),
      file.path(paths$publication_graphs_dir, "NumSigDaMolecules", "metabolites_num_sig_da_molecules.tab")
    )
  }
...
module_ci_metab_summary_render_stub <- function(output_dir, template = "metabolomics_report.rmd") {
...
module_ci_metab_summary_scorecard <- function(project_dirs, session_export_path = NULL, report_path = NULL) {
  paths <- project_dirs$metabolomics
  artifacts <- c(
    study_parameters = file.path(paths$source_dir, "study_parameters.txt"),
    da_workbook = file.path(paths$results_summary_dir, "Publication_tables", "DA_results_metabolomics.xlsx"),
    enrichment_workbook = file.path(paths$results_summary_dir, "Publication_tables", "Pathway_enrichment_results_metabolomics.xlsx"),
...
module_ci_metab_summary_with_project_dirs <- function(project_dirs, code) {
