...
test_that("ci impact workflow exposes detect-impact and dynamic matrix jobs", {
  ...
  expect_match(workflow, "browser-actions/setup-chrome@v1", fixed = TRUE)
  expect_match(workflow, "CHROMOTE_CHROME", fixed = TRUE)
  expect_match(workflow, "steps.chrome.outputs.chrome-path", fixed = TRUE)
