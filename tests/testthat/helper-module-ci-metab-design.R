module_ci_metab_design_paths <- function() {
  root <- tempfile("module-ci-metab-design-")
  paths <- list(
    base_dir = root,
...
module_ci_metab_design_samples <- function(kind = "balanced") {
  switch(
    kind,
...
module_ci_metab_design_assay <- function(samples = module_ci_metab_design_samples(),
                                         feature_col = "Feature.Name",
                                         annotation_col = "Annotation",
                                         feature_prefix = "M",
                                         sample_order = samples) {
  sample_values <- setNames(
    lapply(seq_along(sample_order), function(idx) c(100, 200, 300) + (idx * 10)),
    sample_order
  )
  assay <- data.frame(
    setNames(list(paste0(feature_prefix, sprintf("%03d", 1:3))), feature_col),
    setNames(list(c("Alanine", "Lactate", "Citrate")), annotation_col),
    sample_values,
    check.names = FALSE,
...
module_ci_metab_design_assays <- function(layout = "lc_pair",
                                          samples = module_ci_metab_design_samples(),
  ...
  primary <- module_ci_metab_design_assay(samples = samples, sample_order = assay_samples, feature_prefix = "LC")
  secondary_order <- if (isTRUE(reorder_secondary)) rev(assay_samples) else assay_samples
  secondary <- module_ci_metab_design_assay(samples = samples, sample_order = secondary_order, feature_prefix = "SEC")
...
module_ci_metab_design_column_mapping <- function(samples = module_ci_metab_design_samples(),
                                                  feature_col = "Feature.Name",
                                                  annotation_col = "Annotation") {
  list(
    metabolite_id_col = feature_col,
...
module_ci_metab_design_matrix <- function(kind = "two_group", samples = NULL) {
  if (is.null(samples)) {
    samples <- module_ci_metab_design_samples(if (identical(kind, "three_group")) "three_group" else if (identical(kind, "unbalanced")) "unbalanced" else "balanced")
  }
  ...
  if (identical(kind, "extra_metadata")) {
    ...
    design$age_days <- c(10, 10, 11, 11)[seq_len(nrow(design))]
    design$instrument_batch <- rep(c("MS1", "MS2"), length.out = nrow(design))
  }
  design
}

module_ci_metab_design_contrasts <- function(kind = "valid") {
  switch(
    kind,
...
module_ci_metab_design_config <- function(formula = "~ 0 + group") {
  list(
...
module_ci_metab_design_payload <- function(kind = "two_group",
                                           layout = "lc_pair",
                                           formula = "~ 0 + group",
                                           contrasts = module_ci_metab_design_contrasts("friendly")) {
  design <- module_ci_metab_design_matrix(kind)
  assays <- module_ci_metab_design_assays(layout = layout, samples = design$Run)
  list(
    design_matrix = design,
    data_cln = assays,
    contrasts_tbl = contrasts,
    config_list = module_ci_metab_design_config(formula),
    column_mapping = module_ci_metab_design_column_mapping(design$Run)
  )
}

module_ci_metab_design_write_import_pack <- function(root, payload) {
...
module_ci_metab_design_workflow <- function(payload = NULL) {
  ...
  workflow$config_list <- if (is.null(payload)) module_ci_metab_design_config() else payload$config_list
  workflow$tab_status <- list(setup_import = "complete", design_matrix = "pending", quality_control = "disabled")
  workflow$state_manager <- NULL
  workflow
}

module_ci_metab_design_assert_samples_once <- function(design, expected_samples) {
...
module_ci_metab_design_assert_preflight_passes <- function(payload, require_contrasts = TRUE) {
  result <- validateMetabDesignDaPreflight(
...
module_ci_metab_design_file_nonempty <- function(path) {
