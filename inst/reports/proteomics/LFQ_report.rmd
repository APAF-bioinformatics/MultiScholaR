---
title: "`r paste('LFQ Proteomics Analysis for', params$workflow_name, '- report - created on', params$timestamp)`"
...
# Artifact sessions provide explicit paths; legacy renders retain project_dirs.
library(tidyverse)
library(purrr)
library(flextable)
library(here)
...
current_project_paths <- params$project_paths
if (is.null(current_project_paths)) {
  if (!exists("project_dirs", envir = .GlobalEnv) || !is.list(project_dirs)) {
    stop(paste(
      "Global object 'project_dirs' not found or is not a list.",
      "Run MultiScholaR::setupDirectories() first."
    ))
  }
  ...
  current_project_paths <- tryCatch({
    ...
    stop(glue::glue(paste0(
      "Failed to get project paths for omic_type='{params$omic_type}', ",
      "experiment_label='{params$experiment_label}': {e$message}"
    )))
  })
}
...
# NEW: Enhanced parameter extraction for LFQ workflow
ExtractStudyParametersLFQ <- function(study_params) {
  ...
  # NEW: Extract mixed species FASTA analysis info
  mixed_species_info <- study_params$"Mixed Species FASTA Analysis" %||% list()
  enrichment_filter_info <- study_params$"Enrichment Analysis - Organism Filtering" %||% list()
  ...
  # Extract S4 parameter sections (LFQ-specific - same as TMT since both are protein-level)
  s4_global <- s4_params$globalParameters %||% list()
  s4_prot_filt <- s4_params$removeRowsWithMissingValuesPercent %||% list()
  s4_prot_rep_filt <- s4_params$removeProteinsWithOnlyOneReplicate %||% list()
  s4_accession <- s4_params$chooseBestProteinAccession %||% list()
  ...
  ui_de <- safe_extract(ui_params, "Differential Abundance UI Parameters", list())
  ui_enrich <- safe_extract(ui_params, "Enrichment Analysis UI Parameters", list())
  
  # Create flat parameter list for LFQ workflow
  all_params <- list(
    # Basic Information
    ...
    # RUV Status (check both sections for backwards compatibility)
    ruv_applied = if (!is.null(ruv_batch_correction$"Status")) {
      tolower(trimws(ruv_batch_correction$"Status")) != "not applied"
    } else {
      # Default to TRUE for backwards compatibility with old sessions
      !is.null(ruv_results$"Best k value")
    },
    
    # RUV Auto-optimization Results (only populated when RUV was applied)
    ...
    # LFQ Protein-level filtering parameters (FIXED: explicit NULL check to handle 0 values)
    groupwise_percentage_cutoff = if(is.null(s4_prot_filt$groupwise_percentage_cutoff) || is.na(s4_prot_filt$groupwise_percentage_cutoff)) 60 else as.numeric(s4_prot_filt$groupwise_percentage_cutoff),
    max_groups_percentage_cutoff = if(is.null(s4_prot_filt$max_groups_percentage_cutoff) || is.na(s4_prot_filt$max_groups_percentage_cutoff)) 60 else as.numeric(s4_prot_filt$max_groups_percentage_cutoff),
    proteins_intensity_cutoff_percentile = if(is.null(s4_prot_filt$proteins_intensity_cutoff_percentile) || is.na(s4_prot_filt$proteins_intensity_cutoff_percentile)) 1 else as.numeric(s4_prot_filt$proteins_intensity_cutoff_percentile),
    protein_grouping_variable = s4_prot_rep_filt$grouping_variable %||% "group",
    
    # Protein accession cleanup parameters
    accession_delimiter = s4_accession$delim %||% ";",
    ...
    protein_count_final = protein_counts_info$"Final proteins for DA analysis" %||% NA,
    
    # NEW: Mixed Species FASTA Analysis
    mixed_species_enabled = mixed_species_info$"Multi-Species FASTA Used" %||% "No",
    mixed_species_primary_organism = mixed_species_info$"Selected Primary Organism" %||% NA,
    mixed_species_taxon_id = mixed_species_info$"Selected Taxon ID" %||% NA,
    mixed_species_filtered_at_import = mixed_species_info$"Filtered at Import" %||% "No",
    
    # NEW: Enrichment Organism Filtering
    enrichment_filter_enabled = enrichment_filter_info$"Filter Applied" %||% "No",
    enrichment_filter_proteins_before = enrichment_filter_info$"Proteins Before" %||% NA,
    enrichment_filter_proteins_after = enrichment_filter_info$"Proteins After" %||% NA,
...
# UPDATED: Method details for LFQ workflow - protein-level processing
FormatMethodDetailsLFQ <- function(numbers) {
  # Use LFQ-specific parameters (FIXED: explicit NULL check for 0 values)
  groupwise_cutoff <- if(is.null(numbers$groupwise_percentage_cutoff) || is.na(numbers$groupwise_percentage_cutoff)) 60 else as.numeric(numbers$groupwise_percentage_cutoff)
  max_groups_cutoff <- if(is.null(numbers$max_groups_percentage_cutoff) || is.na(numbers$max_groups_percentage_cutoff)) 60 else as.numeric(numbers$max_groups_percentage_cutoff)
  intensity_cutoff_pct <- if(is.null(numbers$proteins_intensity_cutoff_percentile) || is.na(numbers$proteins_intensity_cutoff_percentile)) 1 else as.numeric(numbers$proteins_intensity_cutoff_percentile)
  ...
  fasta_text <- tryCatch({
    if (!is.null(numbers$fasta_format) && !is.na(numbers$fasta_format) && nzchar(numbers$fasta_format)) {
      ...
      "Protein identifications were validated against the provided FASTA reference database to ensure accurate annotation."
    }
  }, error = function(e) {
    "Protein identifications were validated against the provided FASTA reference database to ensure accurate annotation."
  })
  ...
  accession_cleanup_text <- tryCatch({
    ...
    if (cleanup_applied) {
      ...
      "Protein accessions were cleaned and validated to ensure accurate annotation and to select the best representative protein ID when multiple accessions were present."
    }
  }, error = function(e) {
    "Protein accessions were cleaned and validated to ensure accurate annotation and to select the best representative protein ID when multiple accessions were present."
  })
  ...
  ruv_count_text <- if(!is.null(numbers$protein_count_ruv) && !is.na(numbers$protein_count_ruv)) {
    paste0(" Following RUV-III batch correction and subsequent missing value cleanup, ", 
           numbers$protein_count_ruv, " proteins were retained for differential abundance analysis.")
  } else {
    ""
  }
  
  # Generate cross-group filtering text BEFORE glue block
  cross_groups_text <- if(max_groups_cutoff == 0) {
    "Additionally, proteins were required to pass this threshold in all experimental groups, ensuring comprehensive coverage across all conditions."
  } else {
    paste0("Additionally, proteins were excluded if they failed this threshold in more than ", max_groups_cutoff, "% of experimental groups.")
  }
  
  # Generate RUV text conditionally based on whether RUV was applied
  ruv_applied_val <- numbers$ruv_applied %||% TRUE  # Default to TRUE for backwards compatibility
  
  ruv_text <- if (isTRUE(ruv_applied_val)) {
    glue::glue(
  ...
  # NEW: Generate mixed species FASTA text BEFORE glue block
  mixed_species_text <- ""
  if (!is.null(numbers$mixed_species_enabled) && 
      tolower(numbers$mixed_species_enabled) == "yes") {
    mixed_species_text <- paste0(
      "\n\nMixed-Species FASTA Database\n",
      "The analysis used a mixed-species FASTA database containing proteins from ",
      "multiple organisms. The primary organism was identified as ",
      numbers$mixed_species_primary_organism %||% "unknown",
      " (NCBI Taxonomy ID: ", numbers$mixed_species_taxon_id %||% "N/A", ")."
    )
    
    if (tolower(numbers$mixed_species_filtered_at_import %||% "no") == "yes") {
      mixed_species_text <- paste0(mixed_species_text,
        " Proteins from non-target organisms were filtered at the import stage."
      )
    }
    
    if (tolower(numbers$enrichment_filter_enabled %||% "no") == "yes" &&
        !is.na(numbers$enrichment_filter_proteins_before)) {
      mixed_species_text <- paste0(mixed_species_text,
        " For enrichment analysis, additional organism filtering was applied, ",
        "reducing the protein set from ", numbers$enrichment_filter_proteins_before,
        " to ", numbers$enrichment_filter_proteins_after, " proteins."
      )
    }
  }
...
Data Import and Protein Quantification
Protein-level quantification data was imported from FragPipe or MaxQuant workflow 
output. The data contains label-free quantification (LFQ) values for identified 
proteins across all samples. LFQ-based quantification provides relative abundance 
measurements through intensity-based algorithms (such as MaxLFQ for FragPipe or 
LFQ intensities for MaxQuant), which normalize protein abundances across samples 
without the use of isobaric tags. For FragPipe workflows, the MaxLFQ algorithm 
provides protein-level quantification by normalizing peptide intensities across 
samples. For MaxQuant workflows, LFQ intensities are calculated from peptide-level 
data and provide normalized protein abundances across all samples.

Protein Quality Control and Filtering
Protein identifications were filtered to ensure high-confidence quantification 
and reliable downstream statistical analysis. 

FASTA Reference and Protein Accession Cleanup
{fasta_text}

{accession_cleanup_text} Duplicate protein entries were removed to prevent redundancy in quantification.{mixed_species_text}

Proteins were then filtered based on missing value patterns across experimental 
groups to retain only those with sufficient coverage for robust statistical 
testing. Proteins were excluded if they had {groupwise_cutoff}% or more missing 
values or values below the {intensity_cutoff_pct}st percentile of intensity values 
within any individual experimental group. {cross_groups_text} 
Additionally, proteins with intensity values below the {intensity_cutoff_pct}st 
percentile across all samples were excluded from further analysis as likely noise 
or background signal.

Proteins appearing in only one replicate across all groups were also excluded 
to ensure measurement reproducibility and reliable quantification across biological 
replicates.{qc_count_text}

Normalization and Batch Effect Correction
Using the protein intensity matrix, zero values were replaced with NA to denote 
missing values. The data matrix was log (base 2) transformed and between-sample 
normalization was performed using the '{norm_method}' method from the 'limma' 
R package (Ritchie et al., 2015). This normalization approach adjusts for 
...
using principal component analysis (PCA) plots, density boxplots, relative 
log-expression (RLE) plots, and the distribution of Pearson correlation between 
replicate samples within groups. These quality control visualizations allow 
assessment of technical variation removal while ensuring biological signal is 
retained.
...
# UPDATED: Results function - focuses on correlation data (conditional on RUV status, same for TMT and LFQ)
...
• Log2 normalisation (leftmost column, a, c, e, g)  
• Cyclic loess normalisation (rightmost column, b, d, f, h)  
...
in Figure 3, with a threshold of q-value < {ui_q_val_thresh} and no log fold-change 
...
AnalyzeSamples <- function(design_matrix, study_params) {
  ...
  tryCatch({
    ...
    params_extracted <- ExtractStudyParametersLFQ(study_params)
...
FormatDESummaryTable <- function(filepath) {
  ...
  tryCatch({
    ...
    if (nrow(sig_data) == 0) return("No significant differentially expressed proteins were found.")
    ...
    summary_text <- sig_data %>%
      ...
      dplyr::mutate(
        text = glue::glue("For the comparison {comparison}, there were {up} upregulated and {down} downregulated proteins.")
      ) %>%
      dplyr::pull(text) %>%
      paste(collapse = " ")
      
    return(paste("The number of significant differentially expressed proteins is summarized in Figure 2.", summary_text))
  }, error = function(e) {
...
# UPDATED: Main function using new parser for LFQ
Main <- function() {
  ...
  # Try RUV filename first, then fall back to normalized filename
  corr_path <- if (file.exists(ruv_corr_path)) {
    ruv_corr_path
  } else if (file.exists(norm_corr_path)) {
    norm_corr_path
  } else {
    ruv_corr_path  # Default to RUV path for error message
  }
  ...
  } else { warning(glue::glue("File not found: {corr_path} (also checked: {norm_corr_path})")); list() }
  
  # Run analysis using UPDATED AnalyzeSamples with LFQ parser
  ...
  # Generate text sections using UPDATED Format* functions for LFQ
  report_content <- list(
    content = list(
      sample_details = FormatSampleDetails(analysis_results$numbers),
      method_details = FormatMethodDetailsLFQ(analysis_results$numbers),
...
if(dir.exists(num_sig_dir)) {
  ...
  # Prefer the "with_not_significant" version if it exists
  ...
  if(length(sig_plot_file) > 0) {
    ...
    cat(glue::glue("Warning: No NumSigDA Plot images found in {num_sig_dir}\n"))
  }
} else {
...
cat("*Figure 2: Number of significant differentially expressed proteins for each comparison.*\n\n")
```
...
cat("*Figure 3: Volcano plots of differentially expressed proteins in alphabetical order.*\n\n")
```
...
cat("*Figure 4: GO enrichment analysis showing pathways enriched in differentially expressed proteins. Plots are ordered alphabetically by comparison and direction.*\n\n")
```
