---
title: "`r paste('TMT Proteomics Analysis for', params$workflow_name, '- report - created on', params$timestamp)`"
...
# NEW: Enhanced parameter extraction for TMT workflow
ExtractStudyParametersTMT <- function(study_params) {
  ...
  # Extract S4 parameter sections (TMT-specific)
  ...
  # Create flat parameter list for TMT workflow
  all_params <- list(
    # Basic Information
    ...
    # TMT Protein-level filtering parameters (FIXED: explicit NULL check to handle 0 values)
...
# UPDATED: Method details for TMT workflow - protein-level processing
FormatMethodDetailsTMT <- function(numbers) {
  # Use TMT-specific parameters (FIXED: explicit NULL check for 0 values)
...
Protein-level quantification data was imported from Proteome Discoverer TMT 
workflow output. The data contains isobaric tag-based (TMT) quantification 
values for identified proteins across all samples. TMT-based quantification 
provides relative abundance measurements through reporter ion intensities in 
MS/MS spectra, with proteins already quantified at the protein level by the 
upstream Proteome Discoverer workflow.
...
# UPDATED: Results function - focuses on correlation data (conditional on RUV status, same for TMT and DIA)
...
AnalyzeSamples <- function(design_matrix, study_params) {
  ...
  tryCatch({
    ...
    params_extracted <- ExtractStudyParametersTMT(study_params)
...
# UPDATED: Main function using new parser for TMT
Main <- function() {
  ...
  # Run analysis using UPDATED AnalyzeSamples with TMT parser
  ...
  # Generate text sections using UPDATED Format* functions for TMT
  report_content <- list(
    content = list(
      sample_details = FormatSampleDetails(analysis_results$numbers),
      method_details = FormatMethodDetailsTMT(analysis_results$numbers),
