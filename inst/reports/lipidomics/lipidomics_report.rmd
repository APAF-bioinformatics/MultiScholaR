---
title: "`r paste('Lipidomics analysis for', params$workflow_name, '- report - created on', params$timestamp)`"
...
safe_extract <- function(params, path, default = NULL) {
  if (is.null(params) || is.null(path)) {
    return(default)
  }
...
parse_per_assay_ruv <- function(ruv_section) {
  ...
  if (is.null(ruv_section) || length(ruv_section) == 0) {
    return(result)
  }
...
# Enhanced parameter extraction for lipidomics
ExtractLipidStudyParameters <- function(study_params) {
  ...
  lipid_metadata <- study_params$"Lipidomics Metadata" %||% list()
...
FormatMethodDetails <- function(numbers) {
  ...
  log_text <- if (isTRUE(numbers$log_transform_applied)) {
    ...
    glue::glue("Lipid intensities were log2-transformed with an offset of {offset_val} to stabilize variance.")
  } else {
    ...
    if (!is.null(ruv_assay_list) && length(ruv_assay_list) > 0) {
      ...
      per_assay_sentences <- sapply(names(ruv_assay_list), function(assay_name) {
        ...
        if (!is.na(k_val) && !is.na(ctrl_val)) {
          paste0(
            "For ", assay_name, ", ", k_val, " unwanted components were removed using ",
            ctrl_val, " empirical negative control lipids."
          )
        } else if (!is.na(k_val)) {
      ...
      glue::glue(
        ...
        "negative control lipids, which are features with little or no changes in ",
      ...
      glue::glue(
        ...
        "features as empirical negative controls ({num_neg_ctrl_val} lipids) ",
      ...
      ctrl_text <- if (!is.na(num_neg_ctrl_val)) paste0("using ", num_neg_ctrl_val, " empirical negative control lipids") else "using empirical negative controls"
      glue::glue(
        "To remove batch effects from biological data, the remove unwanted variation ",
        "(RUVIII-C) method was used. The method relies on having a set of endogenous ",
        "negative control lipids, which are features with little or no changes in ",
...
FormatResults <- function(correlation_data,
                          figures_dir = file.path(results_dir, "lipid_qc"),
                          ruv_applied = TRUE) {
...
• High technical reproducibility achieved for majority of lipids
...
FormatOpinions <- function(correlation_data) {
  # Default value
  high_corr_metabolites <- "N/A" # Variable name kept for internal consistency, but content updated if needed
...
The lipidomics data quality is sufficient for differential analysis. The normalization
strategies employed have effectively reduced technical noise while preserving
biological signal.

Recommendations:
• Proceed with differential abundance analysis using the fully normalized dataset
• Consider RUV-adjusted data for downstream multivariate analyses
• Validate top significant lipids using targeted assays if possible
  ",
    .trim = FALSE
  )
}
...
Differential abundance analysis of lipids was performed using the adjusted
...
{treat_desc} Significant differentially abundant lipids were defined as those
with q-values less than {ui_q_val_thresh}.

Volcano plots of differentially abundant lipids across all groups are shown
in Figure 3, with a threshold of q-value < {ui_q_val_thresh}. Full details of all
lipids are provided in the Supplementary results table, DE_lipids_results.xlsx
...
lipids to identify significantly enriched metabolic pathways and biological
...
• The background lipid set consisted of all identified lipids that
...
supplementary file 'Pathway_enrichment_results_lipidomics.xlsx' within the
...
CalculatePatientMetrics <- function(numbers, group_summary) {
  if (is.null(numbers) || is.null(group_summary)) {
    return(numbers)
  }
...
AnalyzeGroups <- function(design_df) {
  if (is.null(design_df) || nrow(design_df) == 0 || !"group" %in% names(design_df)) {
    return(tibble(
      group = character(0), n_samples = numeric(0),
  ...
  tryCatch(
    {
      ...
      params_extracted <- ExtractLipidStudyParameters(study_params)
  ...
  tryCatch(
    {
      ...
      if (nrow(sig_data) == 0) {
        return("No significant differentially abundant lipids were found.")
      }
      ...
      summary_text <- sig_data %>%
        ...
        dplyr::mutate(
          text = glue::glue("For the comparison {comparison}, there were {up} upregulated and {down} downregulated lipids.")
        ) %>%
        dplyr::pull(text) %>%
        paste(collapse = " ")

      return(paste("The number of significant differentially abundant lipids is summarized in Figure 2.", summary_text))
    },
    error = function(e) {
...
Main <- function() {
  ...
  correlation_data <- if (file.exists(corr_path)) {
    tryCatch(
      ...
      error = function(e) {
        warning(glue::glue("Error reading RDS: {e$message}"))
        list()
      }
    )
  } else {
    warning(glue::glue("File not found: {corr_path}"))
    list()
  }
...
cat("*Figure 2: Number of significant differentially abundant lipids for each comparison.*\n\n")
```
...
cat("*Figure 3: Volcano plots of differentially abundant lipids in alphabetical order.*\n\n")
```
...
cat("*Figure 4: Pathway enrichment analysis showing pathways enriched in differentially abundant lipids. Plots are ordered alphabetically by comparison and direction.*\n\n")
```
