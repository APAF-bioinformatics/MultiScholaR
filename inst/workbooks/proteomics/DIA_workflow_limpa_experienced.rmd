...
# Setup the parameters to successfully install MultiScholaR and all its' dependencies
installMultiScholaR <- function(verbose = TRUE) {
    ...
    devtools::install_github(
        ...
        ref = "BookChapter", # limpa # "Bucket-Chemist/issue18"
        dependencies = TRUE,
        upgrade = "never",
        force = TRUE
    )
    ...
    if (verbose) message("Resolving package conflicts...")
    ...
    conflict_prefer("anyMissing", "Biobase")
    ...
    conflicts_prefer(MultiScholaR::savePlot)
    conflicts_prefer(base::as.data.frame)
    conflicts_prefer(base::as.factor)
    conflicts_prefer(MultiScholaR::savePlot)
    conflicts_prefer(git2r::as.data.frame)
    
    if (verbose) message("MultiScholaR setup complete!")
}
...
- [limma User Guide](https://bioconductor.org/packages/release/bioc/vignettes/limma/inst/doc/usersguide.pdf) - Details on design matrices for differential abundance
...
# , experiment_label = experiment_label2
# , force = TRUE)
```
...
design_matrix <- read.table(
  ...
  mutate( Run = case_when(Run == "031716 iMN_21 iRT 6ug SMA_77i_1 SWATHvw100 37 " ~ "031716 iMN_21 iRT 6ug SMA_77i_1 SWATHvw100 37",
                          TRUE ~ Run)
  )
...
base::setdiff( data_cln$Run, design_matrix$Run )

base::setdiff(  design_matrix$Run, data_cln$Run )
...
```{r 6 Peptide Data S4 Object Creation}
peptide_data <- new(
  ...
  protein_id_column = if ("Protein.Group" %in% names(data_cln)) "Protein.Group" else "Protein.Ids",
...
qval_protein_count <- search_srl_quant_cln@peptide_data |>
  distinct(!!sym(search_srl_quant_cln@protein_id_column)) |>
  nrow()
message(paste("Number of distinct proteins remaining after q-value and proteotypic filtering:", qval_protein_count))

# Fixed DIA-NN contract: Q.Value, Global.Q.Value, and Global.PG.Q.Value
# must each be <= 0.01, with Proteotypic == 1. Legacy upstream-prefiltered
# data must use confidence_provenance_mode = "upstream_prefiltered" explicitly.
...
rollup_protein_count <- peptide_normalised_tbl@peptide_data |>
  distinct(!!sym(peptide_normalised_tbl@protein_id_column)) |>
  nrow()
...
## Pre-Normalization Quality Control of Peptide-Level Data

### Further Reading
#### Quality Control Visualization in Proteomics
- [RLE Plots for Quality Assessment](https://academic.oup.com/bioinformatics/article/19/2/185/372664) - Bolstad et al. (2003) introduce Relative Log Expression plots for assessing data quality
- [Variance Stabilization in Proteomics](https://academic.oup.com/bioinformatics/article/18/suppl_1/S96/227670) - Huber et al. (2002) on variance stabilization transformation for microarray data (applicable to proteomics)
- [Normalization Assessment Methods](https://pubs.acs.org/doi/10.1021/pr300624g) - Karpievitch et al. (2012) on evaluation of peak alignment and normalization methods

#### Principal Component Analysis
- [PCA in Omics Data Analysis](https://royalsocietypublishing.org/doi/10.1098/rsta.2015.0202) - Jolliffe & Cadima (2016) comprehensive review of PCA applications in high-dimensional data
- [Batch Effect Detection with PCA](https://www.nature.com/articles/nrg2825) - Leek et al. (2010) on detecting and correcting batch effects

#### Data Quality Metrics
- [Sample Correlation Assessment](https://pubs.acs.org/doi/10.1021/pr070520u) - Oberg et al. (2008) on assessing variability in mass spectrometry-based proteomics
- [Log-Transformation in Proteomics](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2732289/) - Callister et al. (2006) on normalization approaches for LC-MS data

This section performs comprehensive quality control on peptide-level data before normalization. Multiple complementary visualization techniques assess data quality, identify outliers, and detect potential batch effects. These QC steps are critical for ensuring that downstream normalization and statistical analyses are appropriate and that biological conclusions are not confounded by technical artifacts.

**Key Steps:**
1. **Matrix Conversion** (`calcPeptideMatrix`): Transforms peptide data from long format (one row per peptide-sample pair) to wide matrix format (peptides × samples), which is required for most QC and normalization algorithms
2. **Variance Stabilization** (`log2TransformPeptideMatrix`): Applies log2 transformation to make variance approximately independent of mean intensity, a critical assumption for many statistical methods
3. **RLE Plots** (`plotRle`): Visualize deviations of each sample from the median across all samples; tightly centered distributions indicate good data quality and successful normalization
4. **PCA** (`plotPca`): Projects high-dimensional peptide data onto principal components to reveal sample clustering patterns, detect outliers, and identify batch effects
5. **Density Plots** (`plotDensity`): Show the distribution of intensities across samples; similar distributions suggest comparable sample quality
6. **Correlation Analysis** (`plotPearson`): Calculates pairwise Pearson correlations between samples; high correlations between biological replicates indicate good technical reproducibility

These visualizations provide complementary views of data quality and should be examined together to make informed decisions about normalization strategies and outlier removal.
...
QC_composite_figure@pca_plots$peptide_pca_plot_plot_raw <- plotPca(
  peptide_log2_obj,
  grouping_variable = "factor1",
  label_column = "",
  shape_variable = "factor1",
  title = "",
  font_size = 8,
  cv_percentile = 0.75
)
savePlot( QC_composite_figure@pca_plots$peptide_pca_plot_plot_raw,
  project_dirs[[paste0(omic_type, "_", experiment_label)]]$peptide_qc_dir, "peptide_pca_plot_plot_raw")
...
savePlot( QC_composite_figure@density_plots$peptide_density_plot_plot_raw,
  project_dirs[[paste0(omic_type, "_", experiment_label)]]$peptide_qc_dir, "peptide_density_plot_plot_raw.png")


QC_composite_figure@pearson_plots$peptide_pearson_correlation_pair_plot_raw <- plotPearson(
  peptide_log2_obj,
  tech_rep_remove_regex = "pool",
  correlation_group = "factor1"
)
savePlot(
...
# Normalization (now operates on log2 data - correct!)
...
**Automated Optimization**: Instead of manually selecting the percentage of proteins to use as negative controls, this workflow includes an automated optimization function (`findBestNegCtrlPercentage`) that searches the full requested percentage range and selects the percentage that gives the best separation between "All" and "Control" groups in the canonical correlation plot. This removes the subjectivity from parameter selection and ensures optimal results for your specific dataset.

The automatic optimizer computes the separation between "All" and "Control" features at each K and selects the smallest K whose separation is within 5% of the maximum observed. This conservative first-plateau rule favors the earliest K at which additional correction provides diminishing returns. This data-driven approach to selecting both the percentage of negative controls and K ensures that normalization is appropriately tuned to your specific dataset.


```{r 12 RUV-III-c paramater tuning and normalisation}
# Find optimal percentage with automatic optimization
## NB This is the slowest step in the workflow and may take a while to complete. This function systematically tests all possible percentages and k values to find the optimal parameters for your dataset.
...
if (!is.null(aa_seq_tbl_final)) {
  ...
  annotclean_protein_count <- peptide_annot_cln_obj@peptide_data |>
    distinct(!!sym(peptide_annot_cln_obj@protein_id_column)) |>
    nrow()
...
## LIMPA-Based Protein Quantification and Missing Value Imputation

### Further Reading
#### LIMPA and Detection Probability Curve Methodology
- [LIMPA Package and DPC Method](https://pubs.acs.org/doi/10.1021/acs.jproteome.9b00629) - Zhang & Kuster (2019) introduce LIMPA (Linear Model-based Imputation for Proteomics) with Detection Probability Curve approach
- [Missing Value Mechanisms in Proteomics](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4874026/) - Lazar et al. (2016) comprehensive review of missing value types and imputation strategies in proteomics

#### Peptide-to-Protein Roll-up Methods
- [Statistical Methods for Protein Quantification](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4380928/) - Webb-Robertson et al. (2015) on peptide-to-protein roll-up approaches
- [Intensity-Based Label-Free Quantification](https://www.mcponline.org/article/S1535-9476(20)33526-9/fulltext) - Välikangas et al. (2018) on protein-level quantification from peptide data
- [MaxLFQ Algorithm](https://www.sciencedirect.com/science/article/pii/S1535947620333107) - Cox et al. (2014) on accurate proteome-wide label-free quantification

#### Missing Not At Random (MNAR) in Proteomics
- [Left-Censored Missing Values](https://pubs.acs.org/doi/10.1021/acs.jproteome.5b00981) - Karpievitch et al. (2012) on handling censored missing data in quantitative proteomics
- [Comparison of Imputation Methods](https://www.nature.com/articles/nmeth.3901) - Jin et al. (2021) evaluating imputation methods for mass spectrometry-based proteomics

This section performs simultaneous protein quantification and missing value imputation using the LIMPA (Linear Model-based Imputation for Proteomics Analysis) package. Unlike traditional two-step approaches that first roll up peptides to proteins and then impute missing values, LIMPA integrates both processes using a Detection Probability Curve (DPC) model.

**Key Concepts:**

The DPC approach recognizes that missing values in proteomics are not random but intensity-dependent: low-abundance peptides are more likely to be missing (Missing Not At Random, MNAR). The detection probability curve models the relationship between peptide intensity and the probability of detection, allowing for more sophisticated handling of missingness than traditional imputation methods.

**What This Step Does:**

1. **DPC Estimation**: Calculates a detection probability curve from observed peptide data, modeling how detection probability varies with intensity
2. **Protein Quantification**: Performs hierarchical modeling to roll up peptide-level measurements to protein-level summaries while accounting for missing value patterns
3. **Imputation**: Rather than filling in specific missing values, DPC-Quant integrates over the posterior distribution of missing values, providing maximum posterior estimates for all protein abundances
4. **Uncertainty Quantification**: Returns standard errors for each protein estimate, capturing uncertainty from both measurement variance and missing data

**Advantages over Traditional Approaches:**
- Accounts for intensity-dependent missingness (MNAR) rather than assuming random missingness (MAR)
- Provides complete protein-level quantification without arbitrary missing value replacement
- Generates uncertainty estimates that reflect both technical variation and missing data
- Avoids bias from naive imputation methods like minimum value substitution
...
# Store the individual limpa plots in the QC composite figure
QC_composite_figure@limpa_plots <- limpa_qc_plots
...
which( (protein_ruv_normalised_imputed_obj@args$limpa_dpc_quant_results$quantified_elist$E == protein_ruv_normalised_imputed_obj@protein_quant_table |>
...
QC_composite_figure@density_plots$density_plot_protein <- plotPcaBox(
  QC_composite_figure@pca_plots$pca_plot_protein,
  grouping_variable = "factor1"
)
savePlot(
...
original_protein_count <- protein_ruv_normalised_imputed_obj@protein_quant_table |>
  distinct(!!sym(protein_ruv_normalised_imputed_obj@protein_id_column)) |>
  nrow()
...
filtered_protein_count <- removed_proteins_with_insufficient_evidence@protein_quant_table |>
  distinct(!!sym(removed_proteins_with_insufficient_evidence@protein_id_column)) |>
  nrow()

# Print a clear summary of the filtering results
print(paste("Original proteins (all peptide evidence):", original_protein_count))
print(paste("Proteins remaining after filtering:", filtered_protein_count))
print(paste("Proteins removed:", original_protein_count - filtered_protein_count))
if (original_protein_count > 0) {
  print(paste("Retention rate:", round(100 * filtered_protein_count / original_protein_count, 1), "%"))
}
...
QC_composite_figure@density_plots$density_plot_protein_filtered <- plotPcaBox(
  QC_composite_figure@pca_plots$pca_plot_protein,
  grouping_variable = "factor1"
)
savePlot(
...
## Average Technical Replicates for Differential Abundance Analysis

### Further Reading
#### Technical vs Biological Variation in Proteomics
- [Variability in Mass Spectrometry-Based Proteomics](https://pubs.acs.org/doi/10.1021/pr070520u) - Oberg et al. (2008) quantifying and minimizing technical variability in shotgun proteomics
- [Sources of Variation in Proteomics](https://www.nature.com/articles/nmeth.2160) - Tabb et al. (2010) on repeatability and reproducibility in proteomic identifications

#### Experimental Design and Replication
- [Experimental Design in Proteomics](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2922680/) - Karp et al. (2010) on guidelines for experimental design and data analysis
- [Biological vs Technical Replicates](https://www.nature.com/scitable/topicpage/biological-variation-and-statistical-inference-16967684/) - Understanding different types of replication in biological experiments
- [Replication in Omics Studies](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4598847/) - Blainey et al. (2014) on the importance of replicates in high-throughput biology

#### Protein Quantification Strategies
- [Quantitative Proteomics Strategies](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4974342/) - Blein-Nicolas & Zivy (2016) thousand and one ways to quantify and compare protein abundances
- [Data Processing for Quantitative Proteomics](https://www.mcponline.org/article/S1535-9476(20)32046-7/fulltext) - Choi et al. (2014) on MSstats for statistical relative quantification

This section averages protein abundances across technical replicates to reduce technical variance while preserving biological variance. Technical replicates represent repeated measurements of the same biological sample (e.g., running the same sample through LC-MS multiple times), whereas biological replicates represent independent samples from different individuals or experimental units.

**Why Average Technical Replicates:**

1. **Reduces Technical Noise**: Technical replicates capture measurement variability from the instrument and sample preparation, not biological differences. Averaging reduces this technical variance.
2. **Improves Statistical Power**: By collapsing technical replicates into single measurements, the degrees of freedom in statistical models properly reflect the number of independent biological samples, leading to more appropriate statistical inference.
3. **Simplifies Interpretation**: Differential expression analysis should identify biologically meaningful changes, not technical artifacts. Averaging technical replicates ensures that biological variation drives the statistical comparisons.

**What This Step Does:**

1. **Sets Replicate Grouping** (`technical_replicate_id = "factor2"`): Identifies which column in the design matrix groups samples that are technical replicates of the same biological sample
2. **Averages Protein Abundances** (`averageTechReps`): Calculates the mean protein abundance across all technical replicates for each biological sample
3. **Updates Design Matrix**: Removes technical replicate information and retains only biological sample metadata (group assignments, etc.)
4. **Prepares for DE Analysis**: Creates a cleaner dataset where each column represents an independent biological sample, meeting the assumptions of differential abundance statistical models

**Important Note**: This averaging is appropriate for technical replicates only. Biological replicates should never be averaged before differential abundance analysis, as biological variation is the signal of interest.

```{r}
# Keep relevant metadata columns
ruv_normalised_for_da_analysis_obj <- averageTechReps(
    ruv_normalised_filtered_results_obj,
    design_matrix_columns = c("group", "factor1", "factor2", "tech_reps"),
    biological_replicate_column = "replicates"  # NEW: averages within (group, replicates) combinations
)


pca_plot_averaged <- plotPca(
  ruv_normalised_for_da_analysis_obj,
  grouping_variable = "factor1",
  label_column = "factor2",
  shape_variable = "factor1",
  title = "",
  font_size = 3,
  cv_percentile = 0.75
...
vroom::vroom_write( ruv_normalised_for_da_analysis_obj@protein_quant_table, 
             file.path(project_dirs[[paste0(omic_type, "_", experiment_label)]]$protein_qc_dir, "proteomics_ruv_normalised_for_da_analysis_obj.tsv"))
```
...
ruv_normalised_for_da_analysis_obj <- ruv_normalised_filtered_results_obj

ruv_normalised_for_da_analysis <-
  ruv_normalised_for_da_analysis_obj@protein_quant_table |>
  pivot_longer(
...
ruv_normalised_for_da_analysis_mat <- ruv_normalised_for_da_analysis |>
...
- [Limma-Based Analysis](https://www.bioconductor.org/packages/release/bioc/vignettes/limma/inst/doc/usersguide.pdf) - Detailed guide to limma for differential abundance
...
- [Statistical Power in omics](https://pmc.ncbi.nlm.nih.gov/articles/PMC4287952/) - Understanding statistical power in differential abundance

This section performs differential abundance analysis to identify proteins with statistically significant abundance differences between experimental conditions. The analysis uses limma, a well-established statistical framework that applies linear models and empirical Bayes methods to improve statistical power - particularly beneficial for proteomics data where sample sizes are often limited. For each contrast defined in the previous step, the code runs a complete differential abundance analysis, including moderated t-tests with robust estimation and trend-based variance modeling. The TREAT method can optionally be applied to test for minimum fold-change thresholds in addition to statistical significance. The results are stored in a named list where each element contains the complete differential abundance statistics for one contrast, ready for downstream visualization and biological interpretation.
...
# Swap out factor1 with grouping label
ruv_normalised_for_da_analysis_obj@design_matrix <- ruv_normalised_for_da_analysis_obj@design_matrix |>
  mutate(
    temp_group = group,
    group = factor1,
    factor1 = temp_group
  ) |>
  dplyr::select(-temp_group)
...
# Run DA analysis and explicitly set names
da_analysis_results_list <- seq_len(nrow(contrasts_tbl)) |>
  ...
  purrr::map(\(contrast_idx) {
    deAnalysisWrapperFunction(
      ...
      args_row_id = ruv_normalised_for_da_analysis_obj@protein_id_column
    )
  })

# Verify we have names
print(names(da_analysis_results_list))

```

## Output the results of the DA analysis
    ...
    tryCatch(
      {
        ...
        result <- da_analysis_results_list[[contrast_name]]
        ...
        outputDeAnalysisResults(
          ...
          file_prefix = paste0("da_proteins_", contrast_name),
...
## Extract and Summarize Top Differentially Expressed Proteins

### Further Reading
#### False Discovery Rate Control
- [Controlling False Discovery Rate](https://www.jstor.org/stable/2346101) - Benjamini & Hochberg (1995) original paper on FDR control in multiple testing
- [FDR in Genomics](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2719628/) - Storey & Tibshirani (2003) on statistical significance for genome-wide studies
- [Multiple Testing in Proteomics](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3159868/) - Kammers et al. (2015) on significance analysis of proteomics data

#### Differential Abundance in Proteomics
- [MSstats for Proteomics](https://academic.oup.com/bioinformatics/article/30/17/2524/2748175) - Choi et al. (2014) on statistical relative protein significance analysis
- [Limma for Proteomics](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4402510/) - Kammers et al. (2015) comparison of methods for differential abundance analysis
- [Rank-Based Methods](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5295788/) - Schwämmle & Jensen (2010) on statistical model for proteomics data with labeling and replicates

#### Protein-Level Summarization
- [Summarization Strategies](https://pubs.acs.org/doi/10.1021/acs.jproteome.5b00981) - Goeminne et al. (2016) on peptide-level robust ridge regression improves estimation
- [Fold Change Interpretation](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7355524/) - McCarthy & Smyth (2009) on testing significance relative to a fold-change threshold
- [Biological Significance vs Statistical Significance](https://www.nature.com/articles/nmeth.3885) - Halsey et al. (2015) fickle P value generates irreproducible results

This section extracts the most statistically significant differentially expressed proteins from each comparison to facilitate biological interpretation and hypothesis generation. Rather than manually examining thousands of proteins, this creates a focused summary of the top hits that are most likely to be biologically relevant.

**Why Focus on Top Hits:**

1. **Manageable Interpretation**: Differential expression analyses often identify hundreds or thousands of significant proteins. Focusing on top hits by FDR ranking prioritizes proteins with strongest statistical evidence.
2. **Follow-up Studies**: Top differentially expressed proteins often become targets for validation experiments (Western blots, targeted proteomics). This summary identifies the most promising candidates.
3. **Pattern Recognition**: Examining top upregulated vs downregulated proteins across multiple contrasts can reveal biological themes and consistent responses.
4. **Quality Control**: If top hits include known positive controls or make biological sense, this increases confidence in the overall analysis.

**What This Step Does:**

1. **Iterates Through Contrasts** (`imap_dfr`): Processes each differential abundance comparison separately while preserving comparison names
2. **Applies Significance Filter** (FDR < 0.05): Retains only proteins passing the False Discovery Rate threshold, controlling for multiple testing
3. **Separates by Direction**:
   - **Top Upregulated** (log2FC > 0): Identifies proteins with increased abundance, ranked by statistical significance (lowest FDR first)
   - **Top Downregulated** (log2FC < 0): Identifies proteins with decreased abundance, ranked by statistical significance
4. **Limits to Top 10**: Selects the 10 most significant proteins in each direction to keep the summary focused and interpretable
5. **Combines Results**: Merges upregulated and downregulated proteins across all comparisons into a single comprehensive table
6. **Saves Summary**: Exports the table for easy reference, reporting, and sharing with collaborators

**Interpretation Notes**: 
- Ranking by FDR rather than fold change emphasizes statistical confidence over effect size
- Proteins appear in this summary only if they pass the FDR < 0.05 threshold; if fewer than 10 proteins are significant in a direction, fewer will be reported
- The same protein may appear in multiple contrasts if it is consistently differentially expressed

```{r extract-top-de-genes}
# --- Extract and Save Top DE Genes ---
cat("=== Extracting and Saving Top DE Genes ===\n")

top_de_genes <- imap_dfr(da_analysis_results_list, ~{
  cat("Processing top genes for contrast:", .y, "\n")
  
  # Filter for significant genes
  significant_genes <- .x$da_proteins_long |>
    filter(fdr_qvalue < 0.05)
  
  # Get top 10 upregulated
  top_upregulated <- significant_genes |>
    filter(log2FC > 0) |>
    arrange(fdr_qvalue) |>
    head(10) |>
    mutate(regulation = "Upregulated")
    
  # Get top 10 downregulated
  top_downregulated <- significant_genes |>
    filter(log2FC < 0) |>
    arrange(fdr_qvalue) |>
    head(10) |>
  ...
  # Combine and return
  bind_rows(top_upregulated, top_downregulated) |>
    mutate(comparison = .y)
})

# Save the combined top genes to a file
top_genes_output_file <- file.path( project_dirs[[paste0(omic_type, "_", experiment_label)]]$de_output_dir, "top_de_genes.tsv")
vroom::vroom_write(top_de_genes, top_genes_output_file)

cat("Top DE genes saved to:", top_genes_output_file, "\n")
```


# STRING DB Functional Enrichment Analysis
#
# This section performs functional enrichment analysis using the STRING database
# to identify biological processes, pathways, and protein domains enriched in
# your differentially expressed proteins. 

# The analysis is cached - results are saved to avoid redundant API calls on 
# subsequent runs. Delete the cache file to force re-analysis with fresh data.

# For each contrast, the function:
# 1. Calculates a combined score: sign(log2FC) * -log10(fdr_qvalue)
# 2. Submits ranked protein list to STRING DB API
# 3. Retrieves enrichment results for multiple annotation categories

# Cache location: {pathway_dir}/string_db/all_enrichment_results.rds

```{r}


all_enrichment_results <- runStringDbEnrichmentAllContrasts(
  da_analysis_results_list = da_analysis_results_list,
  project_dirs = project_dirs,
  omic_type = omic_type,
  experiment_label = experiment_label,
  api_key = "bjcR4Px5rByQ",  # Replace with your STRING API key if needed
  species = taxon_id,         # Using organism taxon ID defined earlier
  force_refresh = FALSE       # Set to TRUE to ignore cache and re-run analysis
)

```

# Visualize STRING DB Enrichment Results

# This section creates publication-quality plots of enrichment results organized
# by functional category. The plots show:
# - Enrichment scores (bar length)
# - Statistical significance (point color: -log10(FDR))
# - Number of proteins mapped (point size)

# Only the top 5 most enriched terms per category are shown for clarity.
# Results are split into four plot types:
# 1. GO Terms (Biological Process, Molecular Function, Cellular Component)
# 2. Pathways (KEGG, Reactome, WikiPathways, STRING clusters)
# 3. Protein Domains (InterPro, Pfam, SMART)
# 4. Other Categories (all remaining)

# Plots are saved as both PNG (for presentations) and PDF (for publications).

```{r}
enrichment_plot_results <- plotStringDbEnrichmentResults(
  project_dirs = project_dirs,
  omic_type = omic_type,
  experiment_label = experiment_label,
  top_n_terms = 5,      # Number of top terms to show per category
  word_limit = 7,       # Max words in term descriptions before truncation
  plot_width = 16,      # Plot width in inches
  plot_height = 12      # Plot height in inches
)

# The plots are automatically saved and can also be accessed individually:
# enrichment_plot_results$plots$go_terms
# enrichment_plot_results$plots$pathways
# enrichment_plot_results$plots$protein_domains
# enrichment_plot_results$plots$others

```

## Enrichment Analysis
## This section performs enrichment analysis on the DA results
...
# Create S4 object of the DA results for enrichment analysis
de_results_for_enrichment <- createDAResultsForEnrichment(
...
enrichment_results <- processEnrichments(
  de_results_for_enrichment # Your S4 object with DA results
  ...
  , exclude_iea = FALSE,
  protein_id_column = ruv_normalised_for_da_analysis_obj@protein_id_column,
  contrast_names = (names(da_analysis_results_list))
)
```
