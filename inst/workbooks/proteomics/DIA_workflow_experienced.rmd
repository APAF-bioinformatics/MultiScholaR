...
## When you already have MultiScholaR installed, you can start from this point. The code below loads the package and all its dependencies to prepare your environment for analysis.
```{r Load MultiScholaR}
library(MultiScholaR)
loadDependencies()
```
...
## already have it
DIANN_filename <- "KV_DIANN_report.tsv" ## copy to /data/proteomics in your project directory
fasta_filename <- "04153260899A.fasta" ## copy to /data/UniProt in your project directory
uniprot_search_results <- "idmapping_2024_10_07.tsv" ## copy to /data/UniProt in your project directory eg "idmapping.tsv"
uniparc_search_results <- "kv_uniparc.csv" ## copy to /data/UniProt in your project directory eg "idmapping.tsv"
## Please supply your organism's taxon ID here
taxon_id <- 244366 # You can find this at the links above.
## Please supply your organism's name here
organism_name <- "Klebsiella variicola" # If you don't know this by now, you have bigger problems than this workflow!

data_tbl <- vroom::vroom(file.path(project_dirs$proteomics$data_dir, DIANN_filename))
fasta_file_path <- file.path(project_dirs$proteomics$uniprot_annotation_dir, fasta_filename)
...
This section creates a structured S4 object to hold your peptide data with all the necessary metadata. By organizing your data in this object, MultiScholaR can easily access and manipulate the data in subsequent analysis steps, while maintaining the relationship between peptides, proteins, and experimental conditions.
...
rollup_protein_count <- peptide_normalized_tbl@peptide_data |>
  distinct(!!sym(peptide_normalized_tbl@protein_id_column)) |>
  nrow()
...
pif_protein_count <- peptide_normalized_pif_cln@peptide_data |>
  distinct(!!sym(peptide_normalized_pif_cln@protein_id_column)) |>
  nrow()
...
#   theObject      = peptide_normalized_tbl,
#   function_name  = "peptideIntensityFiltering",
#   parameter_name = "min_reps_per_group", # Direct minimum number of distinct observed runs per passing group
#   new_value      = NEW_MIN_REPLICATES_HERE
# )
# peptide_normalized_tbl <- updateConfigParameter(
#   theObject      = peptide_normalized_tbl,
#   function_name  = "peptideIntensityFiltering",
#   parameter_name = "min_groups", # Direct minimum number of biological groups that must pass
#   new_value      = NEW_MIN_GROUPS_HERE
# )
...
twopep_protein_count <- removed_proteins_with_less_than_two_peptides@peptide_data |>
  distinct(!!sym(removed_proteins_with_less_than_two_peptides@protein_id_column)) |>
  nrow()
...
minsamp_protein_count <- peptide_keep_samples_with_min_num_peptides@peptide_data |>
  distinct(!!sym(peptide_keep_samples_with_min_num_peptides@protein_id_column)) |>
  nrow()
...
onerep_protein_count <- removed_peptides_with_only_one_replicate@peptide_data |>
  distinct(!!sym(removed_peptides_with_only_one_replicate@protein_id_column)) |>
  nrow()
...
impute_protein_count <- peptide_values_imputed@peptide_data |>
  distinct(!!sym(peptide_values_imputed@protein_id_column)) |>
  nrow()
...
protein_id_column <- peptide_values_imputed@protein_id_column
peptide_sequence_column <- peptide_values_imputed@peptide_sequence_column
sample_id_column <- peptide_values_imputed@sample_id

iq::process_long_format(
  peptide_values_imputed_file,
  output_filename = file.path(results_dir, "protein_qc", "iq_output_file.txt"),
  sample_id = sample_id_column,
  primary_id = protein_id_column,
  secondary_id = peptide_sequence_column,
...
protein_id_table <- peptide_values_imputed@peptide_data |>
  dplyr::select(dplyr::any_of(unique(c(protein_id_column, "Protein.Ids")))) |>
  dplyr::distinct() |>
  dplyr::filter(.data[[protein_id_column]] %in% protein_log2_quant[[protein_id_column]])
...
protein_obj <- ProteinQuantitativeData(
  ...
  sample_id = peptide_values_imputed@sample_id,
  group_id = peptide_values_imputed@group_id,
  technical_replicate_id = peptide_values_imputed@technical_replicate_id,
...
if (is.null(uniprot_search_results)) {
  ...
  annotclean_protein_count <- protein_log2_quant_cln@protein_quant_table |>
    distinct(!!sym(protein_log2_quant_cln@protein_id_column)) |>
    nrow()
...
protmiss_protein_count <- protein_normalized_pif_cln@protein_quant_table |>
  distinct(!!sym(protein_normalized_pif_cln@protein_id_column)) |>
  nrow()
...
protein_normalized_pif_cln@protein_quant_table |>
  dplyr::distinct(!!sym(protein_normalized_pif_cln@protein_id_column)) |>
  nrow()
```
...
protonerep_protein_count <- remove_proteins_with_only_one_rep@protein_quant_table |>
  distinct(!!sym(remove_proteins_with_only_one_rep@protein_id_column)) |>
  nrow()
...
This section implements the RUV-III (Remove Unwanted Variation) method with canonical correlation analysis to further normalize your data by removing unwanted technical variation while preserving biological signal. The approach uses proteins least likely to be differentially expressed (identified by ANOVA) as "negative controls" to estimate and remove sources of unwanted variation. The canonical correlation plot helps determine the optimal number of factors (k) to remove — too few factors may leave unwanted variation, while too many might remove biological signal. The automatic optimizer computes the separation between "All" and "Control" features at each K and selects the smallest K whose separation is within 5% of the maximum observed. This conservative first-plateau rule favors the earliest K at which additional correction provides diminishing returns. This data-driven approach to selecting k ensures that normalization is appropriately tuned to your specific dataset.
...
# Find the best k (uses first-plateau detection; findBestK() is deprecated)
best_k <- findBestKElbow(cancorplot_r1)
...
ruvfilt_protein_count <- ruv_normalised_results_cln_obj@protein_quant_table |>
  distinct(!!sym(ruv_normalised_results_cln_obj@protein_id_column)) |>
  nrow()
...
corfilt_protein_count <- ruv_normalised_filtered_results_obj@protein_quant_table |>
  distinct(!!sym(ruv_normalised_filtered_results_obj@protein_id_column)) |>
  nrow()
...
uniprot_dat_cln <- getUniprotAnnotations(
  input_tbl = ruv_normalised_for_da_analysis_obj@protein_quant_table,
...
This section reads in predefined contrast specifications from a file, allowing for consistent and reproducible differential abundance analysis. Contrasts define the comparisons between experimental groups (e.g., treatment vs. control) that will be tested for protein abundance differences. By defining these contrasts in a separate file, the workflow becomes more flexible and allows for complex experimental designs. This approach also makes it easy to update the analysis with new comparisons without changing the main workflow code. The contrast strings follow a specific format that will be interpreted by the differential abundance analysis function to create the appropriate statistical tests.
