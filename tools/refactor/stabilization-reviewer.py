...
STRUCTURED_ARGV_METADATA_KEYS = {"label", "phase", "replayMode", "source"}
STRUCTURED_ARGV_SENTINELS = {"argv_fixme"}

STAGING_GENERATED_DIR_PARTS = ("tools", "refactor", "staging")
TOP_LEVEL_COUNT_EXPR_FIX_RE = re.compile(
    r"(exprs\s*<-\s*parse\([^;]+;\s*n\s*<-\s*sum\(vapply\()expr(\s*,\s*function\(expr\))"
)
...
def trim_structured_metadata_tail(argv: list[str]) -> tuple[list[str], str | None]:
    if len(argv) < 4:
        return argv, None
    metadata_tokens = STRUCTURED_ARGV_METADATA_KEYS | STRUCTURED_ARGV_SENTINELS
    for index in range(3, len(argv)):
        if argv[index] not in metadata_tokens:
            continue
        trailing = argv[index:]
        core = trailing
        if core and core[-1] in STRUCTURED_ARGV_SENTINELS:
            core = core[:-1]
        if not core:
            repaired = argv[:index]
            if repaired:
                return repaired, f"Trimmed malformed structured replay metadata tail from argv: {trailing!r}"
            continue
        if len(core) % 2 != 0:
            continue
        if not all(core[offset] in STRUCTURED_ARGV_METADATA_KEYS for offset in range(0, len(core), 2)):
            continue
        repaired = argv[:index]
        if repaired:
            return repaired, f"Trimmed malformed structured replay metadata tail from argv: {trailing!r}"
...
def repair_known_r_replay_command(argv: list[str]) -> tuple[list[str], str | None]:
    if len(argv) != 3 or argv[0] != "Rscript" or argv[1] != "-e":
        return argv, None
    expression = argv[2]
    repaired_expression, replacements = TOP_LEVEL_COUNT_EXPR_FIX_RE.subn(r"\1exprs\2", expression)
    if replacements == 0:
        return argv, None
    repaired = list(argv)
    repaired[2] = repaired_expression
    return repaired, "Repaired historical top-level count replay command to use the parsed `exprs` vector."
...
def normalize_replay_command(command_entry) -> tuple[list[str] | None, str | None, str, dict]:
    ...
    if isinstance(command_entry, dict):
        ...
        if argv_value is not None:
            ...
            argv, trim_note = trim_structured_metadata_tail([token for token in argv_value])
            if trim_note:
                metadata["normalizationNotes"].append(trim_note)
            argv, repair_note = repair_known_r_replay_command(argv)
            if repair_note:
                metadata["normalizationNotes"].append(repair_note)
...
def rerun_test_commands(repo_root: pathlib.Path, commands: list, timeout_ms: int) -> tuple[list[str], list[dict], list[str], list[dict]]:
    ...
    for command in commands:
        ...
        notes.extend(metadata.get("normalizationNotes") or [])
