...
- archival closeout:
  - the monolith split itself completed earlier, but the proteomics lane later
    surfaced residual uncommitted helper files from the extracted S4 family
  - those residual production files were audited, verified, and finalized in
    `e924175` (`Finalize residual proteomics S4 helper extractions`)
  - the resulting proteomics production history is now upstream on
    `origin/janitor` via merge commit `aab75ec`
...
April 17, 2026 closeout note: the S4 family is now genuinely finalized for the
proteomics lane. The residual production slice included `R/allGenerics.R` plus
the extracted `func_prot_s4_*` / `func_pept_s4_*` helper files that had been
left out of an earlier commit-scope calculation. Focused RUV and QC-filtering
tests passed before that residual slice was committed.
