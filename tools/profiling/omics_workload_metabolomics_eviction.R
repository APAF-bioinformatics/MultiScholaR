...
run_dir <- tempfile("metab-eviction-worker-")
...
if (!identical(payload_digest, contract$expected_digests$payload_sha256) ||
    ...
    stop("frozen metabolomics workload digest mismatch", call. = FALSE)
}
...
if (backend == "artifact") {
    ...
    for (index in seq_along(assays)) {
        encoded <- encodeArtifactTable(
            assays[[index]],
            owner = paste("metabolomics eviction", assay_names[[index]])
        )
...
result <- list(
    schema = "multischolar.metabolomics_eviction_measurement",
