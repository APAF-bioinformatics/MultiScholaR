...
proteomicsCleanupRunnerPath <- function() {
    ...
    normalizePath(
        file.path(
            ...
            "tools",
            "profiling",
            "run_omics_publication_proteomics_cleanup.R"
        ),
        mustWork = TRUE
    )
}

.PROTEOMICS_CLEANUP_REPO_ROOT <- normalizePath(
    file.path(dirname(proteomicsCleanupRunnerPath()), "..", ".."),
    mustWork = TRUE
)
...
proteomicsCleanupRunnerArgs <- function(argv) {
...
proteomicsCleanupRunnerMain <- function(argv = commandArgs(trailingOnly = TRUE)) {
    args <- proteomicsCleanupRunnerArgs(argv)
    if (file.exists(args$output)) {
        stop("Cleanup output already exists", call. = FALSE)
    }
    plan <- publicationReadJson(args$plan)
    result <- proteomicsPublicationRunCleanup(plan, execute = args$execute)
