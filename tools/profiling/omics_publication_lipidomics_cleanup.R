lipidCleanupAbort <- function(message) {
    lipidPublicationAbort(paste("cleanup", message), "cleanup_error")
}

lipidCleanupRoot <- function() {
    publicationPath(".omics-publication-workloads", "lipidomics")
}

lipidCleanupLexicalRelative <- function(path, root) {
    prefix <- paste0(normalizePath(root), .Platform$file.sep)
    if (!startsWith(path, prefix)) lipidCleanupAbort("path is not owned")
    ...
    if (!nzchar(relative) || any(parts %in% c("", ".", ".."))) {
        lipidCleanupAbort("path is not canonical")
    }
    relative
}

lipidCleanupWalk <- function(root) {
    walk <- function(path) {
        entries <- list.files(
            path,
            all.files = TRUE,
...
lipidCleanupInventory <- function(root) {
    paths <- lipidCleanupWalk(root)
    records <- lapply(paths, \(path) {
        ...
        list(
            path = lipidCleanupLexicalRelative(path, root),
            type = type,
...
lipidCleanupRetain <- function(record) {
    identical(record$type, "file") && (
...
lipidCleanupCopyRetained <- function(root, archive, inventory) {
    records <- Filter(lipidCleanupRetain, inventory$records)
...
lipidCleanupValidatePlan <- function(plan, root, require_removals = TRUE) {
    ...
    ), "Lipidomics cleanup plan")
    protected <- unlist(plan$protected_paths, use.names = FALSE)
    removals <- vapply(plan$removals, `[[`, character(1), "path")
    valid <- identical(
        plan$schema,
        "multischolar.omics_publication_lipidomics_cleanup_plan"
    ) && identical(plan$schema_version, "1.0.0") &&
        identical(plan$owner_ticket_id, "OMICS-ART-067") &&
        identical(plan$status, "approved_for_execution") &&
        identical(plan$root, ".omics-publication-workloads/lipidomics") &&
    ...
    for (relative in c(protected, removals)) {
        ...
        if (isTRUE(require_removals) && relative %in% removals &&
            !dir.exists(full)) {
            lipidCleanupAbort("removal path is missing")
        }
        ...
        if (!startsWith(
            ...
            lipidCleanupAbort("plan path is not owned")
        }
    }
    ...
    if (!valid || any(overlap)) lipidCleanupAbort("plan differs")
    invisible(plan)
}

lipidCleanupOne <- function(removal, root, archive_root, execute) {
    publicationRequireNames(
        removal,
        c("path", "reason"),
        "Lipidomics cleanup removal"
    )
    source <- file.path(root, removal$path)
    inventory <- lipidCleanupInventory(source)
    record <- list(
        path = removal$path,
    ...
    if (file.exists(archive) || dir.exists(archive)) {
        lipidCleanupAbort("archive already exists")
    }
    dir.create(archive, recursive = TRUE, showWarnings = FALSE)
    record$retained <- lipidCleanupCopyRetained(source, archive, inventory)
    publicationWriteJson(record, file.path(archive, "cleanup-record.json"))
    unlink(source, recursive = TRUE, force = TRUE)
    if (dir.exists(source)) lipidCleanupAbort("removal failed")
    record$removed <- TRUE
    publicationWriteJson(record, file.path(archive, "cleanup-record.json"))
    record
}

lipidPublicationRunCleanup <- function(plan, execute = FALSE) {
    root <- normalizePath(plan$root, mustWork = TRUE)
    lipidCleanupValidatePlan(plan, root)
    archive_root <- file.path(root, plan$archive_root)
    if (isTRUE(execute) && dir.exists(archive_root)) {
        lipidCleanupAbort("archive root already exists")
    }
    records <- lapply(plan$removals, lipidCleanupOne,
        root = root,
    ...
    list(
        schema = "multischolar.omics_publication_lipidomics_cleanup_result",
