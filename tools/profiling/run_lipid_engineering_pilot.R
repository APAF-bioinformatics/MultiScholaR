...
lipidPilotScriptPath <- function() {
    ...
    normalizePath(file.path(
        ...
        "tools",
        "profiling",
        "run_lipid_engineering_pilot.R"
    ))
}

.LIPID_PILOT_ROOT <- normalizePath(
    file.path(dirname(lipidPilotScriptPath()), "..", "..")
)
...
for (source_file in c(
    ...
    source(file.path(.LIPID_PILOT_ROOT, "tools", "profiling", source_file))
}
...
lipidPilotArgs <- function(argv) {
    ...
    while (index <= length(argv)) {
        ...
        if (!startsWith(token, "--") || !key %in% names(values) ||
            index == length(argv)) {
            stop("lipidomics pilot arguments are invalid", call. = FALSE)
        }
        value <- argv[[index + 1L]]
...
lipidPilotRequire <- function(args, fields) {
    missing <- fields[vapply(fields, \(field) {
        is.null(args[[field]]) || !nzchar(args[[field]])
    }, logical(1))]
    if (length(missing)) {
        stop(paste("lipidomics pilot option is missing:", missing[[1L]]),
            call. = FALSE)
    }
    invisible(args)
}

lipidPilotValue <- function(namespace, name) {
    get(name, envir = namespace, inherits = FALSE)
}

lipidPilotPaths <- function(run_dir) {
    root <- file.path(run_dir, "project")
    paths <- list(
        base_dir = root,
        project_id = "lipid-pilot",
        omic_type = "lipidomics",
        omic_label = "lipid-pilot",
...
lipidPilotWorkflow <- function(namespace, paths) {
    workflow <- new.env(parent = emptyenv())
    workflow$workflow_context <- lipidPilotValue(namespace, "createWorkflowContext")(
        paths,
        "lipidomics",
        "lipid-pilot",
        storage_policy = list(
    ...
    workflow$state_manager <- lipidPilotValue(namespace, "WorkflowState")$new()
...
lipidPilotMemoryPayload <- function(namespace, args, mapping) {
    importer <- lipidPilotValue(namespace, "importLipidSearchData")
    assays <- list(
    ...
    list(payload = payload, selection_payload = assays)
}

lipidPilotImport <- function(namespace, workflow, args) {
    mapping <- lipidPilotMapping()
    candidate <- identical(args$arm, "candidate_artifact")
    staged <- NULL
    memory_import <- lipidPilotMemoryPayload(namespace, args, mapping)
    ...
    lipidPilotValue(namespace, "applyLipidImportResultToWorkflow")(
        ...
        lipidIdCol = mapping$lipid_id_col,
        annotationCol = mapping$annotation_col,
        sampleColumns = mapping$sample_columns,
        isPattern = mapping$is_pattern,
        logInfo = \(...) invisible(NULL)
    )
    workflow$processing_log$setup_import <- payload$processingLog
    persist_import <- lipidPilotValue(namespace, "persistLipidImportArtifacts")
    persist_args <- list(
    ...
    if (!isTRUE(persisted$ok) || !isTRUE(persisted$committed)) {
        stop("lipidomics pilot import did not commit", call. = FALSE)
    }
    list(
        payload = payload,
        persisted = persisted,
...
lipidPilotDesign <- function(namespace, workflow, payload) {
    mapping <- payload$columnMapping
    metadata <- c(mapping$lipid_id_col, mapping$annotation_col)
    ...
    object <- lipidPilotValue(namespace, "createLipidomicsAssayData")(
        workflow$data_cln,
    ...
    workflow$state_manager$saveState(
        ...
        "lipidomics engineering pilot"
    )
    persisted <- lipidPilotValue(namespace, "persistLipidDesignArtifacts")(
        workflow,
        log_warn = \(...) invisible(NULL)
    )
    if (!isTRUE(persisted$ok) || !isTRUE(persisted$committed)) {
        stop("lipidomics pilot design did not commit", call. = FALSE)
    }
    list(object = object, persisted = persisted)
}

lipidPilotStateDigest <- function(object, digest_fn) {
...
lipidPilotWorker <- function(args) {
    lipidPilotRequire(args, c(
    ...
    if (fifo_result$status != 0L) stop("lipidomics pilot FIFO creation failed")
    paths <- lipidPilotPaths(args$run_dir)
    workflow <- lipidPilotWorkflow(namespace, paths)
    imported <- lipidPilotImport(namespace, workflow, args)
    design <- lipidPilotDesign(namespace, workflow, imported$payload)
    state_evidence <- lipidPilotStateDigest(
        design$object,
        lipidPilotValue(namespace, "artifactSemanticDigest")
    )
    settlement <- design$persisted$settlement
    if (!is.list(settlement) || !isTRUE(settlement$evicted)) {
        settlement <- lipidPilotValue(
            namespace,
            "settleLipidArtifactWorkflowSafely"
        )(
            workflow,
            rollout_fn = \(...) "evict",
            log_warn = \(...) invisible(NULL)
    ...
    if (!isTRUE(settlement$evicted)) {
        stop("lipidomics pilot settlement failed", call. = FALSE)
    }
    ...
    if (!identical(token, as.raw(1L))) {
        stop("lipidomics pilot acknowledgement differs", call. = FALSE)
    }
    invisible(0L)
}

lipidPilotCategories <- function() {
    list(
...
lipidPilotMeasure <- function(args, arm, repetition) {
    ...
    measurement <- publicationMeasureCgroupSubprocess(
        ...
        unit_name = publicationSystemdUnitName("multischolar-lipid-pilot")
    )
...
lipidPilotRatio <- function(records, metric) {
...
lipidPilotCampaign <- function(args) {
    ...
    for (pair in seq_len(args$repetitions)) {
        ...
        for (arm in order) {
            records[[length(records) + 1L]] <- lipidPilotMeasure(
                args,
                arm,
                pair
            )
    ...
    result <- list(
        schema = "multischolar.lipidomics_engineering_pilot",
        schema_version = "1.0.0",
        owner_ticket_id = "OMICS-ART-076",
        ...
        ), \(metric) lipidPilotRatio(records, metric)) else list(),
...
if (identical(environment(), globalenv()) && !interactive()) {
    args <- lipidPilotArgs(commandArgs(trailingOnly = TRUE))
    status <- tryCatch(
        if (identical(args$mode, "worker")) {
            lipidPilotWorker(args)
        } else {
            lipidPilotCampaign(args)
        },
        error = \(error) {
