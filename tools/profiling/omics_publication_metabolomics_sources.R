metabPublicationForbiddenScaleFields <- function(value) {
    ...
    forbidden <- c(
        ...
        "unique_peptide_count", "assay_mix"
    )
    if (length(intersect(names(value), forbidden))) {
        metabPublicationAbort("metabolomics source leaks cross-omic fields")
    }
    lapply(value, metabPublicationForbiddenScaleFields)
    invisible(TRUE)
}

metabPublicationValidateSources <- function(record) {
    ...
    valid_decisions <- all(vapply(record$decisions, \(decision) {
        ...
        )) && decision$route %in% names(metabPublicationCapabilities()) &&
            identical(
                decision$capability_id,
                metabPublicationCapabilities()[[decision$route]]$capability_id
    ...
    valid_searches <- all(vapply(record$repository_searches, \(search) {
        setequal(names(search), c(
            ...
            "outcome"
        )) && search$route %in% names(metabPublicationCapabilities()) &&
    ...
    valid <- identical(
        record$schema,
        "multischolar.omics_publication_metabolomics_sources"
        ...
        identical(
            ...
            "proteomics_shape_to_bounded_metabolomics_dimensions_only_v1"
        ...
        length(record$repository_searches) == 2L && valid_searches &&
    ...
    metabPublicationForbiddenScaleFields(record$scale_receipt)
    if (!valid) metabPublicationAbort("metabolomics source authority differs")
    invisible(record)
}
