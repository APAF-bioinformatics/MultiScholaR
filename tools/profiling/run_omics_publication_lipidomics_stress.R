...
lipidStressRunnerPath <- function() {
    ...
    normalizePath(
        file.path(
            ...
            "tools",
            "profiling",
            "run_omics_publication_lipidomics_stress.R"
        ),
        mustWork = TRUE
    )
}

.LIPID_STRESS_REPO_ROOT <- normalizePath(
    file.path(dirname(lipidStressRunnerPath()), "..", ".."),
    mustWork = TRUE
)

source(file.path(
    .LIPID_STRESS_REPO_ROOT,
    "tools",
    "profiling",
    "omics_publication_protocol.R"
))

lipidStressRunnerArgs <- function(argv) {
    values <- list(
        unit = NULL,
        contract = NULL,
        payload_root = NULL,
    ...
    while (index <= length(argv)) {
        ...
        if (!startsWith(token, "--") || !key %in% names(values) ||
            index == length(argv)) {
            stop("Lipidomics stress arguments are invalid", call. = FALSE)
        }
        values[[key]] <- argv[[index + 1L]]
        index <- index + 2L
    }
    if (any(vapply(values, is.null, logical(1)))) {
        stop("Lipidomics stress options are incomplete", call. = FALSE)
    }
    if (!grepl("^[a-z0-9-]+$", values$unit)) {
        stop("Lipidomics stress unit name is invalid", call. = FALSE)
    }
    values
}

lipidStressResultField <- function(output, pattern) {
    line <- grep(pattern, output, value = TRUE)
    if (!length(line)) return(NULL)
    trimws(sub(pattern, "", line[[length(line)]]))
}

lipidStressRunnerMain <- function(argv = commandArgs(trailingOnly = TRUE)) {
    args <- lipidStressRunnerArgs(argv)
    if (file.exists(args$output) || file.exists(args$receipt)) {
        stop("Lipidomics stress output already exists", call. = FALSE)
    }
    ...
    systemd_args <- c(
        ...
        paste0("--working-directory=", .LIPID_STRESS_REPO_ROOT),
    ...
    receipt <- list(
        schema = "multischolar.omics_publication_lipidomics_stress_execution",
        ...
        systemd_result = lipidStressResultField(
            combined,
            "^.*Finished with result:[[:space:]]*"
        ),
        service_runtime = lipidStressResultField(
            combined,
            "^.*Service runtime:[[:space:]]*"
        ),
        memory_peak = lipidStressResultField(
            combined,
            "^.*Memory peak:[[:space:]]*"
        ),
    ...
    if (!success) {
        stop("Lipidomics stress import failed", call. = FALSE)
    }
    invisible(0L)
}
